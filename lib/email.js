...
function buildNoNewHtml(noNewApps, checked) {
  const base = baseUrl();
  const scope = checked != null ? `Checked ${checked} tracked application${checked !== 1 ? 's' : ''}.` : '';

  const list = noNewApps.length
    ? `<ul style="font-size:14px;padding-left:20px">${noNewApps.map((a) => {
        const num = a.application_number || a.applicationNumber || '';
        const label = a.label ? ` &mdash; ${esc(a.label)}` : '';
        return `<li><a href="https://patentcenter.uspto.gov/applications/${esc(num)}" style="color:#1a3a6b">${esc(num)}</a>${label}</li>`;
      }).join('')}</ul>`
    : '<p style="font-size:14px;color:#718096">Every tracked application had new filings today (alerts sent to their recipients).</p>';

  return `<div style="font-family:-apple-system,Segoe UI,Roboto,sans-serif;max-width:680px;margin:0 auto;color:#2d3748">
    <h2 style="color:#1a3a6b">USPTO Patent Watch — no new filings today</h2>
    <p style="font-size:14px">${esc(scope)} The following had <strong>no new filings</strong> today:</p>
    ${list}
    ${base ? `<p style="font-size:13px"><a href="${base}" style="color:#1a3a6b">Open the dashboard &rarr;</a></p>` : ''}
    <p style="color:#a0aec0;font-size:12px;margin-top:24px">Automated daily summary from your USPTO file-wrapper tracker.</p>
  </div>`;
}

// Daily summary of applications with NO new filings, sent to DIGEST_TO.
export async function sendNoNewSummary(noNewApps, checked) {
  const to = process.env.DIGEST_TO;
  if (!to) return { skipped: true, reason: 'DIGEST_TO not set.' };
  return postEmail({
    to,
    subject: 'USPTO Patent Watch — no new filings today',
    html: buildNoNewHtml(noNewApps, checked),
  });
}
