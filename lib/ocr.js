...
async function ocrChunk(bytes, wantSearchable = true) {
  ...
  const res = await fetchWithTimeout(OCR_URL, { method: 'POST', body: form }, 25000);
...
export async function ocrTextOfBuffer(buffer, maxChunks = 4) {
  let chunks;
  ...
  let allText = '';
  for (const c of sized.slice(0, maxChunks)) {
    if (c.length > MAX_BYTES) continue;
    const { text } = await ocrChunk(c, false);
    allText += '\n' + (text || '');
  }
  return allText;
}
