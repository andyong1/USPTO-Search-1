...
// The case caption for a court mention, tied to the right case. A court usually
// TRAILS its caption ("Caption Number COURT" tables, "Caption, No. X (COURT)"),
// so when a case number sits just before the court we take the nearest PRECEDING
// "X v. Y"; otherwise the court leads a list ("in the E.D. Tex.: cap1; cap2") and
// we take the nearest FOLLOWING caption. Kept tight so an adjacent case's parties
// don't bleed in. Returns the ~caption text, or null.
function nearestCaption(section, at) {
  const W = 200;
  const trailing = /\d{1,2}[:-]\d{2}-cv-\d{2,6}\s*$|\bno\.\s*\S{0,20}$|\)\s*$/i.test(section.slice(Math.max(0, at - 45), at));
  let vpos = -1;
  if (trailing) {
    const region = section.slice(Math.max(0, at - W), at);
    const ms = [...region.matchAll(/\sv\.?\s/gi)];
    if (ms.length) vpos = Math.max(0, at - W) + ms[ms.length - 1].index;
  } else {
    const m = section.slice(at, at + W).match(/\sv\.?\s/i);
    if (m) vpos = at + m.index;
  }
