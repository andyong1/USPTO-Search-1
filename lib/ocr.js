...
async function ocrChunk(bytes, wantSearchable = true, timeoutMs = 25000, engine = '1') {
...
export async function ocrTextOfBuffer(buffer, maxChunks = 2, perChunkMs = 12000, engine = '1') {
  let chunks;
  ...
  for (let i = 0; i < use.length; i++) {
    ...
    const { text } = await ocrChunk(use[i], false, perChunkMs, engine);
    allText += '\n' + (text || '');
  }
  return allText;
}
