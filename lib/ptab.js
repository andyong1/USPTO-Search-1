...
// Retry+backoff like postDocuments — the USPTO file endpoint returns transient
// 404/429/5xx under load, which would otherwise store an empty/error row and
// mis-classify a real FWD as 'other'. Only a persistent failure throws.
...
export async function fetchIprPetitionCount(from, to, timeoutMs = 25000) {
  for (let i = 0; i < 3; i++) {
    ...
    if ((res.status === 404 || res.status === 429 || res.status >= 500) && i < 2) { await sleep(500 * (i + 1)); continue; }
