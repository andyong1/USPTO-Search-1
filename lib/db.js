...
// ── Petition-decision OCR (AWS Textract) state ─────────────────────
...
export async function setDecisionOcrProcessing(appNum, jobId) {
  await ensureSchema();
  await sql`UPDATE reexam_post_petitions SET decision_ocr_status = 'processing', decision_ocr_job = ${jobId}
            WHERE application_number = ${appNum}`;
}

// Decisions whose Textract job is running and needs collecting.
export async function getDecisionsOcrProcessing(limit) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT application_number, decision_doc_id, decision_ocr_job
    FROM reexam_post_petitions
    WHERE decision_ocr_status = 'processing' AND decision_ocr_job IS NOT NULL
    ORDER BY found_at ASC LIMIT ${limit}`;
  return rows;
}
...
export async function countDecisionsOcrPending() {
  ...
  const { rows } = await sql`
    ...
    WHERE decision_doc_id IS NOT NULL AND (decision_ocr_status IS NULL OR decision_ocr_status = 'processing')`;
  return rows[0] ? rows[0].n : 0;
}
