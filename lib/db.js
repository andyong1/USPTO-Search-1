...
export async function listPostOrderPetitions() {
  ...
  const { rows } = await sql`
    ...
    WHERE p.order_date >= '2025-01-01'
      ...
      -- Once AI-verified, the petition must exist AND cite 325(d).
      AND CASE WHEN p.post_ai_v IS NOT NULL
               THEN p.ai_pet_doc_id IS NOT NULL AND p.ai_pet_325d IS DISTINCT FROM false
    ...
    WHERE LEFT(e.official_date, 10) = ${String(date)}
      AND (e.category NOT LIKE 'post_%'
           OR CASE WHEN p.post_ai_v IS NOT NULL
                   THEN p.ai_pet_doc_id IS NOT NULL AND p.ai_pet_325d IS DISTINCT FROM false AND p.ai_dec_325d IS DISTINCT FROM false
