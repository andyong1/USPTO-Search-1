...
const SCHEMA_VERSION = '2026-07-18.8';
...
export async function getTrialsToExtractPetitionRefs(limit) {
  await ensureSchema();
  // Carry the petitioner name (from the decisions/FWD catalog) so litigation can
  // be split into petitioner-involved vs. other at extraction time.
  const { rows } = await sql`
    SELECT DISTINCT pp.trial_number, coalesce(d.petitioner_name, f.petitioner_name) AS petitioner_name
    FROM patent_proceedings pp
    LEFT JOIN ptab_petition_refs pr ON pr.trial_number = pp.trial_number
    LEFT JOIN ptab_decisions d ON d.trial_number = pp.trial_number
    LEFT JOIN ptab_fwd f ON f.trial_number = pp.trial_number
...
export async function setPetitionRefs(trialNumber, refs, petDocId, litPetitioner, litOther) {
  await sql`
    INSERT INTO ptab_petition_refs (trial_number, pet_doc_id, ref_numbers, lit_petitioner, lit_other, grounds_v, checked_at)
    VALUES (${trialNumber}, ${petDocId || null}, ${refs}, ${litPetitioner || []}, ${litOther || []}, ${GROUNDS_V}, now())
    ON CONFLICT (trial_number) DO UPDATE SET
