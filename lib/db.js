...
export const PREORDER_AI_V = 1;
export async function countPreorderToAiVerify() {
  await ensureSchema();
  const { rows } = await sql`
    ...
    WHERE (petition_doc_id IS NOT NULL OR decision_doc_id IS NOT NULL)
      AND coalesce(preorder_ai_v, 0) < ${PREORDER_AI_V}`;
  return rows[0] ? rows[0].n : 0;
}
...
// Version of the verification prompt/logic; bump to re-verify every row.
export const POST_AI_V = 1;
export async function countPostPetToAiVerify() {
  await ensureSchema();
  const { rows } = await sql`
    ...
    WHERE order_date >= '2025-01-01'
      ...
      AND coalesce(post_ai_v, 0) < ${POST_AI_V}`;
  return rows[0] ? rows[0].n : 0;
}
...
export const FWD_AI_V = 1;
// FWDs with stored decision text but no AI verification at the current version.
export async function getFwdToAiVerify(limit) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT trial_number, decision_text FROM ptab_fwd
    WHERE coalesce(decision_text, '') <> '' AND coalesce(fwd_ai_v, 0) < ${FWD_AI_V}
    ORDER BY fwd_date DESC NULLS LAST LIMIT ${limit}`;
  return rows;
}
export async function countFwdToAiVerify() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT count(*)::int AS n FROM ptab_fwd
    WHERE coalesce(decision_text, '') <> '' AND coalesce(fwd_ai_v, 0) < ${FWD_AI_V}`;
  return rows[0] ? rows[0].n : 0;
}
...
export const LIT_AI_V = 1;
// Rows the regex left empty (both arrays empty) but which have stored front-matter
// to read — the residual set the AI pass targets.
export async function getPetitionsToLitAi(limit) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT pr.trial_number, pr.pet_frontmatter,
           coalesce(d.petitioner_name, f.petitioner_name) AS petitioner_name,
    ...
    WHERE coalesce(pr.pet_frontmatter, '') <> ''
      ...
      AND coalesce(pr.lit_ai_v, 0) < ${LIT_AI_V}
    LIMIT ${limit}`;
  return rows;
}
export async function countPetitionsToLitAi() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT count(*)::int AS n FROM ptab_petition_refs
    WHERE coalesce(pet_frontmatter, '') <> ''
      AND coalesce(array_length(lit_petitioner, 1), 0) = 0
      AND coalesce(array_length(lit_other, 1), 0) = 0
      AND coalesce(lit_ai_v, 0) < ${LIT_AI_V}`;
  return rows[0] ? rows[0].n : 0;
}
...
export const CERT_AI_V = 1;
// Certificates with cached OCR text but no AI verification at the current version.
// Carries the regex parse so the verifier can be spot-compared if desired.
export async function getCertsToAiVerify(limit) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT application_number, cert_doc_id, cert_text
    FROM reexam_conclusions
    WHERE cert_doc_id IS NOT NULL AND coalesce(cert_text, '') <> '' AND coalesce(cert_ai_v, 0) < ${CERT_AI_V}
    ORDER BY cert_date DESC NULLS LAST
    LIMIT ${limit}`;
  return rows;
}
export async function countCertsToAiVerify() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT count(*)::int AS n FROM reexam_conclusions
    WHERE cert_doc_id IS NOT NULL AND coalesce(cert_text, '') <> '' AND coalesce(cert_ai_v, 0) < ${CERT_AI_V}`;
...
export async function listUnclassifiedPetitionCodes() {
  await ensureSchema();
  const { rows } = await sql`
...
export async function getPetitionSupersetGaps() {
  await ensureSchema();
  const { rows } = await sql`
