...
export async function recordPetition(p) {
  await ensureSchema();
  await sql`
    INSERT INTO reexam_petitions (application_number, document_identifier, petition_date, order_date, page_count)
    VALUES (${p.applicationNumber}, ${p.documentIdentifier}, ${p.petitionDate || null}, ${p.orderDate || null}, ${p.pageCount || null})
    ON CONFLICT (application_number, document_identifier) DO UPDATE SET
      petition_date = COALESCE(EXCLUDED.petition_date, reexam_petitions.petition_date),
      order_date    = COALESCE(EXCLUDED.order_date, reexam_petitions.order_date),
      page_count    = COALESCE(EXCLUDED.page_count, reexam_petitions.page_count)`;
}
...
export async function getPetitionsToParse(limit) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT application_number, document_identifier
    FROM reexam_petitions WHERE parsed = false
    ORDER BY found_at ASC
    LIMIT ${limit}`;
  return rows;
}

export async function setPetitionParsed(appNum, docId, is325d, score) {
  await ensureSchema();
  await sql`
    UPDATE reexam_petitions SET is_325d = ${is325d}, petition_score = ${score == null ? null : score}, parsed = true
    WHERE application_number = ${appNum} AND document_identifier = ${docId}`;
}

// All post-grant petitions for the dedicated page — ONE row per control number
// (the document most likely to be the petition itself, not an exhibit): highest
// petition-likeness score, then fewest pages, then earliest. Sorted newest-first.
export async function listPostGrantPetitions() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT * FROM (
      SELECT DISTINCT ON (p.application_number)
             p.application_number, p.document_identifier, p.petition_date, p.order_date,
             p.is_325d, p.parsed, p.page_count, p.petition_score,
             d.examiner_name, d.group_art_unit
      FROM reexam_petitions p
      LEFT JOIN (
      ...
      ORDER BY p.application_number, p.petition_score DESC NULLS LAST, p.page_count ASC NULLS LAST, p.petition_date ASC NULLS LAST
    ) t
    ORDER BY t.petition_date DESC NULLS LAST, t.application_number DESC`;
  return rows;
}
