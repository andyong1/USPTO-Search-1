...
// Direct-download links to the remedy orders, for the outcomes column. Per
// investigation, the most-recent MIRRORED (public R2/Blob copy) exclusion order
// and cease-and-desist order — so a GEO/LEO or CDO chip can link straight to the
// PDF. Non-mirrored orders are omitted (they get no link). Keyed by number.
export async function listRemedyOrderLinks() {
  await ensureItcSchema();
  const { rows } = await sql`
