...
const SCHEMA_VERSION = '2026-08-10.2';
...
// Requester-side scorecard: how often each firm's request was ordered vs denied.
// Denominator is only proceedings whose determination we actually attributed, so
// it is a rate over decided requests, not over everything the firm ever filed.
export async function getRequesterFirmStats({ minTotal = 1, filerType = null } = {}) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT requester_firm_key AS firm_key,
           min(requester_firm) AS firm,
           max(requester_filer_type) AS filer_type,
           count(*)::int AS total,
           count(*) FILTER (WHERE determination_kind = 'order')::int AS ordered,
           count(*) FILTER (WHERE determination_kind = 'denial')::int AS denied,
           min(determination_date) AS first_determination,
           max(determination_date) AS last_determination
      FROM reexam_firm
     WHERE requester_firm_key IS NOT NULL AND requester_firm_key <> ''
       AND (${filerType}::text IS NULL OR requester_filer_type = ${filerType})
     GROUP BY requester_firm_key
    HAVING count(*) >= ${minTotal}
     ORDER BY count(*) DESC, requester_firm_key`;
  return rows;
}

// Owner-side counterpart: how often a firm defending a patent saw the request denied.
export async function getOwnerFirmStats({ minTotal = 1 } = {}) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT owner_firm_key AS firm_key, min(owner_firm) AS firm,
           max(owner_filer_type) AS filer_type, count(*)::int AS total,
           count(*) FILTER (WHERE determination_kind = 'denial')::int AS denied,
           count(*) FILTER (WHERE determination_kind = 'order')::int AS ordered
      FROM reexam_firm
     WHERE owner_firm_key IS NOT NULL AND owner_firm_key <> ''
     GROUP BY owner_firm_key HAVING count(*) >= ${minTotal}
     ORDER BY count(*) DESC, owner_firm_key`;
  return rows;
}
