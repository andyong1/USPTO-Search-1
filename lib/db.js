...
export async function ensureSchema() {
  ...
  // Whether the pre-order petition decision references a substantial new question
  // of patentability (SNQ), detected text-first with an OCR fallback. status:
  // NULL = untouched, 'pending_ocr' = no text layer (needs OCR), 'done', 'failed'.
  await sql`ALTER TABLE reexam_preorder ADD COLUMN IF NOT EXISTS decision_snq boolean`;
  await sql`ALTER TABLE reexam_preorder ADD COLUMN IF NOT EXISTS decision_snq_status text`;
  ...
  const { rows } = await sql`
    SELECT * FROM (
      SELECT DISTINCT ON (p.application_number)
             ...
             p.petition_doc_id, p.petition_date, p.decision_doc_id, p.decision_date, p.decision_outcome, p.decision_snq,
...
export async function updatePreorderPetition(appNum, petition, decision) {
  await ensureSchema();
  const decId = decision ? decision.id : null;
  await sql`UPDATE reexam_preorder SET
      petition_doc_id  = ${petition ? petition.id : null},
      petition_date    = ${petition ? petition.date : null},
      decision_doc_id  = ${decId},
      decision_date    = ${decision ? decision.date : null},
      decision_outcome = ${decision ? decision.outcome : null},
      -- When the decision document changes, clear any prior SNQ result so the
      -- new decision is re-checked (compares the OLD decision_doc_id to the new).
      decision_snq        = CASE WHEN decision_doc_id IS DISTINCT FROM ${decId} THEN NULL ELSE decision_snq END,
      decision_snq_status = CASE WHEN decision_doc_id IS DISTINCT FROM ${decId} THEN NULL ELSE decision_snq_status END
    WHERE application_number = ${appNum}`;
...
// ── Pre-order decision SNQ detection (text-first, OCR fallback) ────
// Mirrors the post-order §325(d) state machine, keyed by application number.
// Cron pool: never-touched decisions, for the fast text-only pass.
export async function getPreorderDecisionsToCheckSnq(limit) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT DISTINCT application_number, decision_doc_id
    FROM reexam_preorder
    WHERE decision_doc_id IS NOT NULL AND decision_snq_status IS NULL
    ORDER BY application_number LIMIT ${limit}`;
  return rows;
}
// Image-only decisions awaiting OCR (text pass found no text layer).
export async function getPreorderDecisionsPendingSnqOcr(limit) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT DISTINCT application_number, decision_doc_id
    FROM reexam_preorder
    WHERE decision_doc_id IS NOT NULL AND decision_snq_status = 'pending_ocr'
    ORDER BY application_number LIMIT ${limit}`;
  return rows;
}
export async function setPreorderSnqDone(appNum, isSnq) {
  await ensureSchema();
  await sql`UPDATE reexam_preorder SET decision_snq = ${isSnq}, decision_snq_status = 'done'
            WHERE application_number = ${appNum}`;
}
export async function setPreorderSnqPendingOcr(appNum) {
  await ensureSchema();
  await sql`UPDATE reexam_preorder SET decision_snq_status = 'pending_ocr' WHERE application_number = ${appNum}`;
}
export async function setPreorderSnqFailed(appNum) {
  await ensureSchema();
  await sql`UPDATE reexam_preorder SET decision_snq_status = 'failed' WHERE application_number = ${appNum}`;
}
// Outstanding work for the SNQ backfill (untouched + pending_ocr).
export async function countPreorderSnqPending() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT count(DISTINCT application_number)::int AS n FROM reexam_preorder
    WHERE decision_doc_id IS NOT NULL AND (decision_snq_status IS NULL OR decision_snq_status = 'pending_ocr')`;
  return rows[0] ? rows[0].n : 0;
}
export async function resetFailedPreorderSnq() {
  await ensureSchema();
  const { rowCount } = await sql`UPDATE reexam_preorder SET decision_snq_status = NULL WHERE decision_snq_status = 'failed'`;
  return rowCount;
}
