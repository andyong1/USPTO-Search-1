...
const SCHEMA_VERSION = '2026-07-25.3';
  ...
  await sql`
    INSERT INTO reexam_nirc_art
      (application_number, req_refs, nirc_refs, matches, req_count, nirc_count,
       match_count, match_invalidating, match_distinguished, basis, ai_confidence, ai_note, nirc_ai_v)
    VALUES (${appNum}, ${JSON.stringify(reqRefs)}, ${JSON.stringify(nircRefs)}, ${JSON.stringify(matches)},
            ...
            ${o.basis || null}, ${o.confidence || null}, ${o.note || null}, ${NIRC_AI_V})
    ON CONFLICT (application_number) DO UPDATE SET
      ...
      nirc_ai_v = EXCLUDED.nirc_ai_v, updated_at = now()`;
}
...
export async function listNircArt() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT n.application_number, n.nirc_doc_id, n.nirc_date,
