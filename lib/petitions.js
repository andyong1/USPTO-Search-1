...
export function classifyPetitionDoc(code, desc) {
  ...
  // PET.POA.WDRW = "Request for withdrawal as attorney or agent" (37 CFR 1.36).
  // A genuine request for relief -- withdraw_as_attorney is already in the relief
  // vocabulary, and we were already capturing the DECISIONS on these (they issue
  // under PETDEC / RXPTGR / RXPTDE / PET.DEC.AUTO, all recognized) while dropping
  // the requests themselves. Unlike RXRQ/T there is no separate decision code to
  // add: verified on 90015397, where the request drew a plain PETDEC 11 days later.
  if (c === 'PET.POA.WDRW') return { kind: 'petition' };
...
// Oppositions answer a petition on a hard rhythm: across the corpus the
// petition -> opposition gap is 14 days at the 25th, 50th AND 75th percentile,
// and 19 at the 90th. Only 6 of 195 pairs exceed 45 days.
//
// Those outliers are mispairings, not slow oppositions. 90/015,704 is the
// confirmed case: the paper filed 57 days later opposes a patent owner filing
// that never appears in the wrapper (it was not entered by the Office), so FIFO
// attached it to the only petition in sight. The relationship it asserted --
// that this petition drew an opposition -- was simply false.
//
// So a gap past the window means "not an answer to that petition" and the two
// are left unpaired. Deliberately one-directional: an opposition is never
// re-attached to some other petition, only detached. If the threshold is wrong
// we understate a relationship rather than assert one that does not exist.
const OPPOSITION_MAX_GAP_DAYS = 45;

/** Days between two YYYY-MM-DD dates, or null when either is missing/unparseable. */
...
/**
 * Which open petition, if any, an opposition is answering.
 *
 * Evidence first: an opposition names its target in the caption ("Opposition to
 * Patent Owner's Petition filed March 30, 2026"), and petopp classification
 * stores that date. When we have it, it decides the question outright --
 * including the decision that NOTHING here is the target, which is the whole
 * point. The 90/015,704 opposition names a patent owner paper the Office never
 * entered into the wrapper, so no open petition matches and it stays unpaired on
 * evidence rather than on a date heuristic.
 *
 * A party check follows: nobody opposes their own petition. Where both papers
 * are attributed, a same-party candidate is disqualified.
 *
 * The timing window is the fallback for the unclassified majority.
 */
function pickOpposed(open, opp) {
  const free = [...open].reverse().filter((x) => !x.opposition && x.petition);
  if (!free.length) return null;
  const target = ymd(opp.opp_opposes_date);
  if (target) {
    // An exact hit wins wherever it sits in the stack, and a stated target that
    // matches nothing means the opposed paper is not in the wrapper.
    return free.find((x) => ymd(x.petition.official_date) === target) || null;
  }
  const oppParty = opp.opp_party;
  for (const x of free) {
    const petParty = x.petition.req_party;
    if (oppParty && petParty && oppParty !== 'unclear' && petParty !== 'unclear' && oppParty === petParty) continue;
    const gap = dayGap(x.petition.official_date, opp.official_date);
    if (gap === null || gap <= OPPOSITION_MAX_GAP_DAYS) return x;
    // The nearest candidate is already outside the window; earlier ones are worse.
    break;
  }
  return null;
}
