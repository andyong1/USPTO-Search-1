...
const SCHEMA_VERSION = '2026-07-18.2';
...
// PTAB side: trial number -> extracted references.
export async function getFwdGroundsMap() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT trial_number, ref_numbers FROM ptab_fwd
    WHERE coalesce(grounds_v, 0) >= ${GROUNDS_V} AND ref_numbers IS NOT NULL`;
  const map = new Map();
  for (const r of rows) map.set(r.trial_number, r.ref_numbers || []);
  return map;
}
