...
export async function detectTechCenterForApp(controlNumber) {
  ...
  // Patent owner = applicant of record; fall back to the latest ownership-transfer
  // assignee when the (often older) application lists no applicant.
  let owner = meta.applicant || '';
  if (!owner) owner = await fetchAssignmentOwner(parentApp).catch(() => '');
