...
export async function ensureSchema() {
  ...
  // Patent-owner pre-order SNQ submissions (document code RX.PRO.PO).
...
export async function listPreorder() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT application_number, document_identifier, official_date, filing_date
    FROM reexam_preorder
    WHERE filing_date >= ${PREORDER_CUTOFF}
    ORDER BY official_date DESC NULLS LAST, filing_date DESC`;
  return rows;
}
