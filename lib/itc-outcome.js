...
export function selectMirrorDocs(docs) {
  ...
  // The complaint (type "Complaint" with "complaint" in the title — excludes
  // appendices/exhibits) and the notice of investigation.
  for (const d of pub) {
    const t = String(d.document_type || '').toLowerCase(), ti = String(d.document_title || '').toLowerCase();
    if (t === 'complaint' && /complaint/.test(ti)) add(d, 'complaint');
    if (/notice of investigation/.test(ti)) add(d, 'notice_of_investigation');
  }
  return [...picks.values()];
}
