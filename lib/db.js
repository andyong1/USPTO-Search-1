...
export async function listWatched() {
  ...
  const { rows } = await sql`
    SELECT application_number, label, recipients, created_at
...
export async function addWatched(appNum, label, recipients) {
  await ensureSchema();
  await sql`
    INSERT INTO watched_applications (application_number, label, recipients)
    VALUES (${appNum}, ${label || null}, ${recipients || null})
    ON CONFLICT (application_number)
    DO UPDATE SET label = EXCLUDED.label, recipients = EXCLUDED.recipients`;
}
