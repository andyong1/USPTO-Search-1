...
// Requires the (d) subsection specifically (DA-8): Section 325 also has other
// subsections (estoppel etc.), so a bare "section 325" is NOT a 325(d) signal.
export function detect325d(text) {
  if (!text) return false;
  return /\b325\s*\(\s*d\s*\)/i.test(text);
}
