...
const SCHEMA_VERSION = '2026-07-18.4';
...
export async function setDocGrounds(docId, refs, trials) {
  await sql`UPDATE reexam_doc_text
            SET ref_numbers = ${refs}, trial_mentions = ${trials}, grounds_v = ${GROUNDS_V}
            WHERE doc_id = ${docId}`;
}
...
export async function getReexamGroundsMap() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT application_number, ref_numbers, trial_mentions FROM reexam_doc_text
    WHERE coalesce(grounds_v, 0) >= ${GROUNDS_V}`;
  const map = new Map();
  for (const r of rows) {
    const e = map.get(r.application_number) || { refs: new Set(), trials: new Set() };
  ...
  const out = new Map();
  for (const [k, v] of map) out.set(k, { refs: [...v.refs], trials: [...v.trials] });
  return out;
}
