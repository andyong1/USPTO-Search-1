...
import { extractPdfText, detect325d, detectSnq } from './reexamOutcome.js';
...
// Determine whether a pre-order petition decision references a substantial new
// question of patentability (SNQ). Same text-first / OCR-fallback approach as the
// 325(d) check. With allowOcr=false (the hourly cron) an image-only decision
// returns { isSnq: null, method: 'pending-ocr' } so the slow OCR is left to the
// backfill. Throws on a transient download failure so the caller can retry.
export async function detectPreorderDecisionSnq(appNum, docId, { allowOcr = true, downloadMs = 20000, ocrChunks = 2 } = {}) {
  const { buffer } = await fetchDocumentBytes(appNum, docId, 'PDF', downloadMs);
  let text = '';
  try { text = await extractPdfText(buffer); } catch { text = ''; }
  if (text && text.replace(/\s/g, '').length >= 100) {
    return { isSnq: detectSnq(text), method: 'text' };
  }
  if (!allowOcr) return { isSnq: null, method: 'pending-ocr' };
  if (!ocrTextConfigured()) return { isSnq: false, method: 'none' };
  text = await ocrTextOfBuffer(buffer, ocrChunks);
  return { isSnq: detectSnq(text), method: 'ocr' };
}
