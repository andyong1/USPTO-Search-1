...
export function analyzePetition(docs, preorderDate) {
  ...
  if (petition) {
    for (const d of docs) {
      if (!(d.documentCode || '').toUpperCase().startsWith('RXPTD')) continue;
      if (!decision || parseISO(d.officialDate) < parseISO(decision.officialDate)) decision = d;
    }
  }
  let outcome = null;
  if (decision) {
    const code = (decision.documentCode || '').toUpperCase();
    const desc = (decision.description || '').toLowerCase();
    if (code === 'RXPTDI' || /dismiss/.test(desc)) outcome = 'dismissed';
    else if (/grant/.test(desc)) outcome = 'granted';
    else outcome = 'other';
  }
  return {
