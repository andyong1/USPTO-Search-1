...
const ITC_SCHEMA_VERSION = '2026-07-29.1';
...
export async function listOutcomes() {
  ...
  const { rows } = await sql`
    SELECT investigation_number, ai_disposition, ai_violation, ai_remedies,
           ai_commission_action, ai_confidence, ai_note
...
export async function investigationsForParties(limit, v, number) {
  await ensureItcSchema();
  const n = number || null;
  const { rows } = await sql`
    SELECT DISTINCT ON (d.investigation_number)
           d.investigation_number, d.id AS noi_id, d.mirror_url
    FROM itc_document d
  ...
  return rows.slice(0, limit).map((r) => ({ number: r.investigation_number, noiId: r.noi_id, url: r.mirror_url }));
}
