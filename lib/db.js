...
export async function listNircArt() {
  ...
  const { rows } = await sql`
    SELECT n.application_number,
           ...
           coalesce(n.art_source, 'nirc') AS art_source, n.oa_doc_id, n.oa_date, n.oa_code,
           c.outcome_summary, c.cert_doc_id,
           d.group_art_unit, d.examiner_name
    FROM reexam_nirc_art n
    JOIN reexam_conclusions c ON c.application_number = n.application_number
    LEFT JOIN (
      SELECT DISTINCT ON (application_number) application_number, group_art_unit, examiner_name
