...
export function parseReexamOutcome(text) {
  ...
  if (!amended) {
    // "Claims X ... as amended" — allow garbled/extra words (OCR) between the claim
    // list and "as amended", bounded to one sentence (no period).
    const m = t.match(new RegExp(`claims?\\s+${CLAIMS}\\b[^.]{0,40}as amended`, 'i'));
