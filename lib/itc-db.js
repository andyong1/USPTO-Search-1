...
const ITC_SCHEMA_VERSION = '2026-07-27.2';
...
export const KEY_DOC_PATTERNS = [
  '%notice of investigation%', '%institution of investigation%',
  '%initial determination%', '%commission opinion%', '%commission determination%',
  '%commission notice%', '%limited exclusion order%', '%general exclusion order%',
  '%cease and desist order%', '%consent order%', '%complaint under section 337%',
];

// Public, key-type documents not yet mirrored (mirror_url IS NULL) — the resumable
// mirror queue. Newest first; optionally scoped to one investigation number.
export async function keyPublicDocsToMirror(limit, number) {
  await ensureItcSchema();
  const p = KEY_DOC_PATTERNS;
  const { rows } = number
    ? await sql`SELECT id, investigation_number, document_type, document_title
                FROM itc_document
                WHERE investigation_number = ${number} AND lower(security_level) = 'public' AND mirror_url IS NULL
                  AND (document_type ILIKE ANY(${p}) OR document_title ILIKE ANY(${p}))
                ORDER BY received_date DESC NULLS LAST LIMIT ${limit}`
    : await sql`SELECT id, investigation_number, document_type, document_title
                FROM itc_document
                WHERE lower(security_level) = 'public' AND mirror_url IS NULL
                  AND (document_type ILIKE ANY(${p}) OR document_title ILIKE ANY(${p}))
                ORDER BY received_date DESC NULLS LAST LIMIT ${limit}`;
  return rows;
}
...
export async function countKeyPublicDocsToMirror(number) {
  ...
  const p = KEY_DOC_PATTERNS;
  const { rows } = number
    ? await sql`SELECT count(*)::int AS n FROM itc_document
                WHERE investigation_number = ${number} AND lower(security_level) = 'public' AND mirror_url IS NULL
                  AND (document_type ILIKE ANY(${p}) OR document_title ILIKE ANY(${p}))`
    : await sql`SELECT count(*)::int AS n FROM itc_document
                WHERE lower(security_level) = 'public' AND mirror_url IS NULL
