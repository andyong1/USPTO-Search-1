...
export async function detectActionsForApp(appNum, orderDate) {
  ...
  let nonf = null, finl = null;
  for (const d of docs) {
    if (!onOrAfter(d)) continue;
    if (isNonFinal(d) && (!nonf || parseISO(d.officialDate) < parseISO(nonf.officialDate))) nonf = d;
    if (isFinal(d) && (!finl || parseISO(d.officialDate) < parseISO(finl.officialDate))) finl = d;
  }
  await recordActions(appNum, { orderDate, nonfDate: nonf && nonf.officialDate, finlDate: finl && finl.officialDate });
