...
import { fetchContinuity, fetchMetaData, fetchAssignmentOwner } from './uspto.js';
...
export async function detectTechCenterForApp(controlNumber) {
  ...
  // Patent owner = the CURRENT owner: the assignee of the latest ownership-transfer
  // assignment (skipping security interests). Fall back to the applicant of record
  // (owner at filing) when no ownership assignment is recorded.
  let owner = await fetchAssignmentOwner(parentApp).catch(() => '');
  if (!owner) owner = meta.applicant || '';
