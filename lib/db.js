...
export async function recordPetition(p) {
  await ensureSchema();
  await sql`
    INSERT INTO reexam_petitions (application_number, document_identifier, petition_date, order_date)
    VALUES (${p.applicationNumber}, ${p.documentIdentifier}, ${p.petitionDate || null}, ${p.orderDate || null})
...
export async function setPetition325d(appNum, docId, is325d) {
  await ensureSchema();
  await sql`
    UPDATE reexam_petitions SET is_325d = ${is325d}, parsed = true
    WHERE application_number = ${appNum} AND document_identifier = ${docId}`;
}

// All post-grant petitions for the dedicated page, with examiner/art unit context.
export async function listPostGrantPetitions() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT p.application_number, p.document_identifier, p.petition_date, p.order_date,
           p.is_325d, p.parsed, d.examiner_name, d.group_art_unit
