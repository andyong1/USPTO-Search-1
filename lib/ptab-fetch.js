...
import { isPetitionDoc } from './grounds.js';
...
export async function fetchFwdPage({ types = [], from, to, offset = 0, limit = 100, timeoutMs = 25000 }) {
  ...
  // A rejected server-side filter must FAIL, not silently broaden (DA-10): the
  // old no-query fallback made `count`/`fetched` describe a wider population,
  // corrupting pagination cursors and reconciliation counts.
  const data = await postDecisions({ q: qParts.join(' AND '), rangeFilters, sort, pagination: { offset, limit } }, timeoutMs);
...
// Locate a proceeding's operative PETITION document (the source of the asserted
// prior-art grounds — present for every trial regardless of outcome). Returns
// { docId, url, filingDate } or null. Excludes replies/responses/oppositions,
// which also contain the word "petition(er)". A corrected petition supersedes the
// original but cites the same art, so the earliest petition-type doc is fine.
export async function fetchPetitionDoc(trial, timeoutMs = 20000) {
  const data = await postDocuments({
    filters: [{ name: 'trialNumber', value: [trial] }],
    sort: [{ field: 'documentData.documentFilingDate', order: 'Asc' }],
    pagination: { offset: 0, limit: 100 },
  }, timeoutMs);
  return pickPetitionDoc(data.patentTrialDocumentDataBag || []);
}
// The operative petition from a trial's document list. An EXHIBIT can itself be
// a prior petition (e.g. Samsung filing Apple's 2019 petition as an exhibit in
// IPR2025-01250) — its TITLE reads "Petition for Inter Partes Review…", so an
// exhibit-typed document must be skipped no matter what its title says.
export function pickPetitionDoc(recs) {
  for (const r of recs) {
    const d = r.documentData || {};
    const type = d.documentTypeDescriptionText || '', title = d.documentTitleText || '';
    if (/exhibit/i.test(type)) continue;
    if (isPetitionDoc(type) || isPetitionDoc(title)) {
      return { docId: d.documentIdentifier || '', url: d.fileDownloadURI || '', filingDate: d.documentFilingDate || '' };
    }
  }
  return null;
}
...
export async function fetchInstitutionPage({ from, to, offset = 0, limit = 100 } = {}) {
  ...
  for (const r of recs) {
    ...
    // Preserve the -in-part nuance (DA-14): grouped with granted in rates, but
    // no longer silently collapsed.
    const inst_type = /grant/i.test(m.docType)
      ? (/part/i.test(m.docType) ? 'granted_in_part' : 'granted')
      : (/den(y|ied|ial)/i.test(m.docType) ? 'denied' : 'other');
...
export async function fetchReexamFilingCount(from, to) {
  ...
  }, 12000, { retry404: 1 }); // a transient 404 must not record a false zero (DA-3)
  return data.count ?? 0;
}
...
// All PTAB proceedings (IPR/PGR/CBM, ANY year) for a patent — for the IPR→reexam
// pipeline, which must see proceedings that predate the 2024 catalog cutoff. Queries
// the proceedings API by patentOwnerData.patentNumber with NO date or type filter.
// status = trialStatusCategory. Returns [] when the API 404s (no matches).
export async function fetchProceedingsByPatent(patent, { limit = 50, timeoutMs = 25000 } = {}) {
  const pat = String(patent || '').replace(/[^0-9A-Za-z]/g, '');
  if (!pat) return [];
  for (let i = 0; i < 3; i++) {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), timeoutMs);
    ...
    try {
      res = await fetch(`${BASE}/trials/proceedings/search`, {
        method: 'POST',
        headers: { 'X-API-KEY': getApiKey(), 'Content-Type': 'application/json', Accept: 'application/json' },
        body: JSON.stringify({ q: `patentOwnerData.patentNumber:"${pat}"`, pagination: { offset: 0, limit } }),
        signal: controller.signal,
      });
    } catch (e) {
      clearTimeout(timer);
      if (i < 2) { await sleep(500 * (i + 1)); continue; }
      throw new Error(`PTAB proceedings-by-patent fetch failed: ${e.name === 'AbortError' ? 'timed out' : e.message}`);
    }
    clearTimeout(timer);
    if (res.status === 404) return [];
    if (res.ok) {
      const data = await res.json();
      return (data.patentTrialProceedingDataBag || []).map((r) => {
        const tm = r.trialMetaData || {}, po = r.patentOwnerData || {};
        return {
          trial_number: r.trialNumber || '',
          patent_number: po.patentNumber || pat,
          trial_type: tm.trialTypeCode || '',
          petition_date: tm.petitionFilingDate || '',
          institution_date: tm.institutionDecisionDate || '',
          status: tm.trialStatusCategory || '',
        };
      }).filter((x) => x.trial_number);
    }
    if ((res.status === 429 || res.status >= 500) && i < 2) { await sleep(500 * (i + 1)); continue; }
    throw new Error(`PTAB proceedings-by-patent fetch failed: HTTP ${res.status}`);
  }
  return [];
}
