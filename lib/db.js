...
export async function getTrialsToExtractPetitionRefs(limit) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT u.trial_number, max(u.petitioner_name) AS petitioner_name
    FROM (
      SELECT trial_number, petitioner_name FROM ptab_decisions
      UNION ALL SELECT trial_number, petitioner_name FROM ptab_fwd
      UNION ALL SELECT trial_number, NULL::text FROM patent_proceedings
    ) u
...
export async function setPetitionRefs(trialNumber, refs, petDocId, litPetitioner, litOther, frontmatter) {
  ...
  await sql`
    INSERT INTO ptab_petition_refs (trial_number, pet_doc_id, ref_numbers, lit_petitioner, lit_other, pet_frontmatter, grounds_v, lit_v, checked_at)
    VALUES (${trialNumber}, ${petDocId || null}, ${cleanArr(refs)}, ${cleanArr(litPetitioner)}, ${cleanArr(litOther)}, ${fm || null}, ${GROUNDS_V}, ${LIT_V}, now())
    ON CONFLICT (trial_number) DO UPDATE SET
      ...
      pet_frontmatter = EXCLUDED.pet_frontmatter, grounds_v = EXCLUDED.grounds_v, lit_v = EXCLUDED.lit_v, checked_at = now()`;
}
...
export async function getPetitionsToRelit(limit) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT pr.trial_number, pr.pet_frontmatter, coalesce(d.petitioner_name, f.petitioner_name) AS petitioner_name
...
export async function countTrialsToExtractPetitionRefs() {
  ...
  const { rows } = await sql`
    ...
    LEFT JOIN ptab_petition_refs pr ON pr.trial_number = u.trial_number
    WHERE coalesce(u.trial_number,'') <> '' AND (pr.trial_number IS NULL OR coalesce(pr.grounds_v,0) < ${GROUNDS_V})`;
  return rows[0] ? rows[0].n : 0;
}
