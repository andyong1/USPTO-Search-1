...
export async function detectCertificateOutcome(appNum, certDocId, nircDocId, { allowOcr = true, downloadMs = 20000, ocrChunks = 3 } = {}) {
  ...
  if (layerLen >= 100) {
    return { outcome: parseReexamOutcome(text), method: 'text', textLen: layerLen };
  }
  if (!allowOcr) return { outcome: null, method: 'pending-ocr', textLen: layerLen };
  if (!ocrTextConfigured()) return { outcome: null, method: 'none', textLen: layerLen };
  text = await ocrTextOfBuffer(buffer, ocrChunks);
  const ocrLen = text ? text.replace(/\s/g, '').length : 0;
  return { outcome: parseReexamOutcome(text), method: 'ocr', textLen: ocrLen };
}
