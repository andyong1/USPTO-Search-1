...
export function parseReexamOutcome(text) {
  ...
  if (!confirmed) {
    // "The patentability of claims X is confirmed." OCR commonly reads "of" as
    // "ot"/"o1", so allow one stray letter after the "o".
    const m = t.match(new RegExp(`patentability o[ft]?\\s+claims?\\s+${CLAIMS}\\s+(?:is|are)\\s+confirmed`, 'i'));
    if (m) confirmed = tidy(m[1]);
    // OCR sometimes drops the trailing "confirmed" entirely. When the certificate
    // states "NO AMENDMENTS ... MADE TO THE PATENT" (the all-confirmed boilerplate),
    // the patentability-of-claims list is a full confirmation.
    else if (/no amendments?\b[^.]{0,40}made to the patent/i.test(t)) {
      const m2 = t.match(new RegExp(`patentability o[ft]?\\s+claims?\\s+${CLAIMS}`, 'i'));
      if (m2) confirmed = tidy(m2[1]);
    }
  }
