...
// Download the raw bytes of a single document (for archiving). Returns
// { buffer, contentType }. Throws on non-200.
...
export async function fetchDocumentBytes(appNum, documentId, format = 'PDF', timeoutMs = 25000) {
  ...
  return { buffer, contentType: res.headers.get('content-type') || '', ext };
}
