...
async function ocrChunk(bytes) {
  ...
  form.append('file', new Blob([bytes], { type: 'application/pdf' }), 'decision.pdf');
  ...
  if (data.SearchablePDFURL) {
    try { const r = await fetch(data.SearchablePDFURL); if (r.ok) searchablePdf = Buffer.from(await r.arrayBuffer()); } catch { /* text-only */ }
  }
