...
export default async function handler(req, res) {
  ...
  try {
    ...
    // discretionary, AND FWD catalog history (one page per feed per run, DA-10),
    ...
    try {
      ...
      decisionsSweep = {};
      for (const feed of ['inst', 'dd', 'fwd']) {
        if (Date.now() > swDeadline) break;
        const key = 'dscan_off_' + feed;
        const off = parseInt((await getPtabKv(key)) || '0', 10) || 0;
        const page = feed === 'fwd' ? await fetchFwdPage({ types: ['IPR', 'PGR', 'CBM'], offset: off, limit: 100 })
          : feed === 'dd' ? await fetchDdPage({ offset: off, limit: 100 }) : await fetchInstitutionPage({ offset: off, limit: 100 });
        for (const r of page.rows) { try { if (feed === 'fwd') await upsertPtabFwdMeta(r); else if (feed === 'dd') await upsertPtabDd(r); else await upsertPtabInstitution(r); } catch { /* skip bad row */ } }
