...
// Owner daily digest: every relevant document newly detected since the last
// digest, grouped into the five site categories. Sent to OWNER_DIGEST_TO.
export async function sendOwnerDigest(events, opts = {}) {
  const to = process.env.OWNER_DIGEST_TO;
  if (!to) return { skipped: true, reason: 'OWNER_DIGEST_TO not set.' };
  if (!events || !events.length) return { skipped: true, reason: 'No new documents.' };
  ...
  const sub = (title, list) => list.length ? `<h4 style="margin:14px 0 6px;font-size:13px;color:#4a5568">${esc(title)} (${list.length})</h4>${table(list)}` : '';
  const section = (title, inner) => inner ? `<div style="margin:0 0 26px"><h3 style="margin:0 0 4px;font-size:15px;color:#1a3a6b">${esc(title)}</h3>${inner}</div>` : '';
  ...
  const sections =
    section('1. Reexam Determinations', det.length ? table(det) : '') +
    section('2. Reexam Office Actions', (nonf.length || finl.length) ? (sub('Non-Final (RXR.NF)', nonf) + sub('Final (RXR.F)', finl)) : '') +
    section('3. Reexam Certificates', cert.length ? table(cert) : '') +
    section('4. Pre-Order SNQ Submissions', (pre.length || prePet.length || preDec.length) ? (sub('Pre-order submissions', pre) + sub('Requestor petitions to respond', prePet) + sub('Petition decisions', preDec)) : '') +
    section('5. Post-Order § 325(d) Petitions', (pp.length || ppOpp.length || ppDec.length) ? (sub('Patent owner petitions', pp) + sub('Requester oppositions', ppOpp) + sub('Petition decisions', ppDec)) : '');

  const total = events.length;
  const label = opts.dateLabel ? ` (${opts.dateLabel})` : '';
  const html = `<div style="font-family:-apple-system,Segoe UI,Roboto,sans-serif;max-width:720px;margin:0 auto;color:#2d3748">
    <h2 style="color:#1a3a6b">USPTO reexam watch &mdash; ${total} new document${total !== 1 ? 's' : ''}${esc(label)}</h2>
    <p style="color:#718096;font-size:14px">New documents added to the site's analytics since the last digest, by category.</p>
    <p style="color:#718096;font-size:13px"><strong>Note:</strong> a document dated tomorrow won't be retrievable until tomorrow.</p>
    ${sections || '<p>No new documents.</p>'}
    ${base ? `<p style="font-size:13px"><a href="${base}/reexam" style="color:#1a3a6b">Open the site &rarr;</a></p>` : ''}
    <p style="color:#a0aec0;font-size:12px;margin-top:24px">Automated daily owner digest.</p>
  </div>`;
  const subject = `USPTO reexam watch: ${total} new document${total !== 1 ? 's' : ''}${label}`;
  return postEmail({ to, subject, html });
}
