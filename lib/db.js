...
export async function recordActions(appNum, a) {
  await ensureSchema();
  await sql`
    INSERT INTO reexam_actions (application_number, order_date, nonf_date, finl_date, checked_at)
    VALUES (${appNum}, ${a.orderDate || null}, ${a.nonfDate || null}, ${a.finlDate || null}, now())
    ON CONFLICT (application_number) DO UPDATE SET
...
export async function listReexamActions() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT a.application_number, a.order_date, a.nonf_date, a.finl_date,
