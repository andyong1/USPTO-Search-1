...
export async function recordDetermination(d) {
  ...
  const { rowCount } = await sql`
    INSERT INTO reexam_determinations
      (application_number, document_identifier, determination_code, determination_type, official_date)
    VALUES (${d.applicationNumber}, ${d.documentIdentifier}, ${d.code}, ${d.type}, ${d.officialDate})
...
export async function listRecentDeterminations(limit = 200) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT application_number, document_identifier, determination_type, official_date, found_at
