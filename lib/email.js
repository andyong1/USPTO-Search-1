  ...
  const table = (list) => `<table style="border-collapse:collapse;width:100%;background:#fff">
    <thead><tr>
      ...
      return `<tr>
        ...
        <td style="padding:6px 10px;border-top:1px solid #e2e8f0;font-size:13px">${esc(e.label || e.doc_code || '')}${e.doc_code ? ` <span style="color:#718096">(${esc(e.doc_code)})</span>` : ''}${e.d325_summary ? `<div style="margin-top:4px;font-size:12px;color:#4a5568;line-height:1.45"><strong>&sect;325(d):</strong> ${esc(e.d325_summary)} <span style="color:#a0aec0">(AI-generated &mdash; verify against the order)</span></div>` : ''}</td>
...
// 6th section: PTAB Director-discretionary + Board institution decisions issued
// the prior day. items: { trial, type, patent, po, kind ('Discretionary' |
// 'Institution'), decision ('deny'|'refer'|'granted'|'denied'), pdfUrl }.
function ptabDecisionsSection(items) {
  const wrap = (inner) => `<div style="margin:0 0 26px"><h3 style="margin:0 0 4px;font-size:15px;color:#1a3a6b">6. PTAB Discretionary &amp; Institution Decisions</h3>${inner}</div>`;
  if (!items || !items.length) return wrap('<p style="color:#718096;font-size:13px;margin:4px 0 0">None.</p>');
  const base = baseUrl();
...
// 7th section: PTAB final written decisions issued the prior day. Each row links
// to its /trial page, the challenged patent (Google Patents), and the decision PDF
// (via the site's proxy). ptab items: { trial, type, patent, po, petitioner, pdfUrl }.
function ptabSection(ptab) {
  const head = '<h3 style="margin:0 0 4px;font-size:15px;color:#1a3a6b">7. PTAB Final Written Decisions</h3>';
  if (!ptab || !ptab.length) return `<div style="margin:0 0 26px">${head}<p style="color:#718096;font-size:13px;margin:4px 0 0">None.</p></div>`;
  const base = baseUrl();
  const th = (t) => `<th style="text-align:left;padding:6px 10px;background:#edf2f7;font-size:12px;color:#4a5568">${t}</th>`;
  const cell = (c, nowrap) => `<td style="padding:6px 10px;border-top:1px solid #e2e8f0;font-size:13px${nowrap ? ';white-space:nowrap' : ''}">${c}</td>`;
  ...
  return `<div style="margin:0 0 26px">${head}<p style="color:#718096;font-size:12px;margin:0 0 6px">Outcome is not yet classified on the day of issuance — open the trial page for the parsed result.</p>${table}</div>`;
}
...
export async function sendComprehensiveDigestTo(email, events, opts = {}) {
  ...
  const base = baseUrl();
  const total = events.length + ptabDecisions.length + ptabDecisionEvents.length;
  const label = dateLabel ? ` (${dateLabel})` : '';
  const body = comprehensiveSections(events) + ptabDecisionsSection(ptabDecisionEvents) + ptabSection(ptabDecisions);
  const html = `<div style="font-family:-apple-system,Segoe UI,Roboto,sans-serif;max-width:720px;margin:0 auto;color:#2d3748">
    ...
    <p style="color:#718096;font-size:14px">Relevant ex parte reexamination filings issued the previous day (by category), plus any new PTAB discretionary/institution decisions and final written decisions.</p>
