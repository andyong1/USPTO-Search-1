...
export async function detectCertificateOutcome(appNum, certDocId, { allowOcr = true, downloadMs = 20000, ocrChunks = 3, ocrEngine = '1' } = {}) {
  ...
  if (!text || text.replace(/\s/g, '').length < 100) {
    ...
    text = await ocrTextOfBuffer(buffer, ocrChunks, 12000, ocrEngine);
    method = ocrEngine === '2' ? 'ocr2' : 'ocr';
  }
