...
export async function resetConclusionParse() {
  ...
  const { rowCount } = await sql`
    ...
    WHERE parsed = true AND outcome_summary IS NULL
      AND (cert_doc_id IS NOT NULL OR nirc_doc_id IS NOT NULL)`;
  return rowCount;
}

// Re-pool ALL conclusions with a certificate/NIRC so they re-parse — e.g. after
// improving the certificate parser. ?outcomes=1&reparse=1.
export async function resetAllConclusionParse() {
  await ensureSchema();
  const { rowCount } = await sql`
    UPDATE reexam_conclusions SET parsed = false
    WHERE cert_doc_id IS NOT NULL OR nirc_doc_id IS NOT NULL`;
  return rowCount;
}
