...
export function sliceRelatedMattersAll(text) {
  ...
  const heads = [
    ...t.matchAll(/related\s+matters?|42\.8\s*\(\s*b\s*\)\s*\(\s*2\s*\)/gi),
    ...t.matchAll(/related\s+district[- ]court|other\s+proceedings/gi),
  ];
...
export function petitionFrontmatter(fullText) {
  ...
  const tiers = [
    /related\s+matters?|42\.8\s*\(\s*b\s*\)\s*\(\s*2\s*\)/gi,
    /related\s+district[- ]court|other\s+proceedings/gi,
  ];
  const cands = [];
...
export function extractRelatedLitigation(text, petitionerName, poName) {
  ...
  for (const { section, found } of sliceRelatedMattersAll(raw)) {
    const r = extractFromSection(section, found, petTok, poTok);
