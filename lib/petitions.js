...
import { recordPostPetition, markPetitionScan, setPetitionDecision325d } from './db.js';
...
// Download a petition DECISION PDF and flag whether it includes a 325(d) analysis.
// Marks it parsed on success; leaves it for retry if the download fails. (Office
// decisions are usually text PDFs; scanned ones yield no text and stay unflagged.)
export async function parsePetitionDecision(row) {
  let buffer;
  try { ({ buffer } = await fetchDocumentBytes(row.application_number, row.decision_doc_id, 'PDF')); }
  catch (e) { throw new Error(`download failed: ${e.message || e}`); }
  const text = await extractPdfText(buffer);
  const is325d = text ? detect325d(text) : false;
  await setPetitionDecision325d(row.application_number, is325d);
  return is325d;
}
