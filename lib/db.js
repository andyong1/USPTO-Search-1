...
export async function ensureSchema() {
  ...
  // Timestamp of the last owner digest send — the window boundary for "new docs".
  await sql`ALTER TABLE reexam_state ADD COLUMN IF NOT EXISTS owner_digest_at timestamptz`;
...
// Documents first detected since the given timestamp, for the owner digest.
// Post-order petition papers are limited to proceedings that still display on the
// § 325(d) page (petition cites 325(d) — and decision cites it, or none yet).
export async function getDocEventsSince(sinceISO) {
  await ensureSchema();
  const { rows } = await sql`
    ...
    WHERE e.discovered_at >= ${sinceISO}
...
export async function getOwnerDigestAt() {
  await ensureSchema();
  const { rows } = await sql`SELECT owner_digest_at FROM reexam_state WHERE id = 1`;
  return rows[0] ? rows[0].owner_digest_at : null;
}

export async function setOwnerDigestAt(ts) {
  await sql`UPDATE reexam_state SET owner_digest_at = ${ts} WHERE id = 1`;
}
