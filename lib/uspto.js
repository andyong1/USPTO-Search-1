...
// RXPET. ("Receipt of Petition in a Reexam") is party-agnostic — the patent
// owner's petitions carry it too — so among the in-window candidates, prefer
// the one filed the same day as (else nearest to) the requester's RX.PRO.RR
// reply: that petition asks leave to file the reply, so it is the requester's.
export function analyzePetition(docs, preorderDate) {
  const pd = parseISO(preorderDate);
  const replyDates = docs
    .filter((d) => (d.documentCode || '').toUpperCase() === 'RX.PRO.RR')
    .map((d) => parseISO(d.officialDate))
    .filter((t) => !isNaN(t));
  const distToReply = (t) => replyDates.length ? Math.min(...replyDates.map((r) => Math.abs(t - r))) : NaN;
  let petition = null, petDist = NaN;
  if (!isNaN(pd)) {
    for (const d of docs) {
      ...
      if (Math.abs(t - pd) <= 20 * 86400000) {
        const dist = distToReply(t);
        const sameDist = dist === petDist || (isNaN(dist) && isNaN(petDist));
        const better = !petition
          || (!isNaN(dist) && (isNaN(petDist) || dist < petDist))
          || (sameDist && (t < parseISO(petition.officialDate)
              || (t === parseISO(petition.officialDate) && String(d.documentIdentifier) < String(petition.documentIdentifier))));
        if (better) { petition = d; petDist = dist; }
      }
    }
  }
  ...
  for (const d of docs) {
    ...
    // Same-day ties (several petitions decided together) break on document id so
    // the pick is stable across runs regardless of the feed's ordering — a
    // flip-flopping pick would spuriously reset the AI-verification gate.
    if (!decision || t < parseISO(decision.officialDate)
        || (t === parseISO(decision.officialDate) && String(d.documentIdentifier) < String(decision.documentIdentifier))) decision = d;
  }
...
export async function fetchTransactions(appNum, timeoutMs = 10000) {
  ...
  // Invalid JSON is a failed pull, not an empty transaction history (DA-3):
  // returning [] here silently weakened downstream classification.
  const data = await res.json().catch(() => { throw new Error(`USPTO transactions fetch failed for ${appNum}: invalid JSON`); });
...
// opts.retry404 (DA-3): a transient 404 is indistinguishable from a real
// zero-match, so count-critical callers retry once with backoff before
// accepting 404 as "zero results" (a false zero silently corrupts statistics).
export async function searchApplications(body, timeoutMs = 12000, { retry404 = 0 } = {}) {
  for (let attempt = 0; ; attempt++) {
    ...
    if (res.status === 404) {
      if (attempt < retry404) { await new Promise((r) => setTimeout(r, 500 * (attempt + 1))); continue; }
    ...
    return validateSearchShape(await res.json());
  }
}

// Response-contract validator (DA-3/DA-16): a well-formed JSON response that is
// missing the expected records array is an API-contract change, NOT an empty
// result — treating it as empty silently produces false zeros. Pure + exported
// so it's unit-testable.
export function validateSearchShape(data) {
  if (!data || typeof data !== 'object') throw new Error('USPTO search: malformed response (not an object)');
  if (data.patentFileWrapperDataBag !== undefined && !Array.isArray(data.patentFileWrapperDataBag)) {
    throw new Error('USPTO search: unexpected response shape (patentFileWrapperDataBag is not an array)');
  }
  if (data.patentFileWrapperDataBag === undefined && data.count === undefined) {
    throw new Error('USPTO search: unexpected response shape (no records array and no count)');
  }
  return data;
}

// Ex parte reexamination determination document codes (order / denial). Shared,
// case-normalized (DA-12): a differently-cased code from the API must not be
// silently dropped.
// RX.SE.ORDER (probe-verified, DA-4): a series-96 supplemental examination that
// raises a SNQ results in a Director-ordered ex parte reexamination under
// 35 U.S.C. 257(b) — it dockets RX.SE.ORDER, never RXREXO/RXREXD.
export const DET_CODES = { 'RXREXO': 'Reexam Ordered', 'RXREXD': 'Reexam Denied', 'RX.SE.ORDER': 'Reexam Ordered (Suppl. Exam)' };
export function determinationLabel(code) {
  return DET_CODES[String(code || '').trim().toUpperCase()] || null;
}
...
export async function fetchMetaData(appNum, timeoutMs = 10000) {
  ...
  return {
    ...
    grantDate: md.grantDate || md.patentIssueDate || '',
    // Patent owner = applicant of record on the underlying application.
...
// From an ODP assignmentBag, return the current owner: the assignee of the
// LATEST true ownership-transfer assignment. Security interests, releases,
// licenses, and liens are skipped (their "assignee" is a lender/agent, not the
// owner — e.g. "HERCULES CAPITAL, INC., AS AGENT" on a SECURITY INTEREST).
const SKIP_CONVEYANCE = /SECURITY|RELEASE|LIEN|COLLATERAL|LICENSE/;
const OWNER_CONVEYANCE = /ASSIGNMENT OF ASSIGNOR|NUNC PRO TUNC|CORRECTIVE ASSIGNMENT|MERGER|CHANGE OF NAME/;
export function pickAssignmentOwner(bag) {
  if (!Array.isArray(bag)) return '';
  const owns = bag.filter((a) => {
    const t = String(a && a.conveyanceText || '').toUpperCase();
    return t && !SKIP_CONVEYANCE.test(t) && OWNER_CONVEYANCE.test(t);
  });
  if (!owns.length) return '';
  owns.sort((a, b) => String(a.assignmentRecordedDate || a.recordedDate || '').localeCompare(String(b.assignmentRecordedDate || b.recordedDate || '')));
  const latest = owns[owns.length - 1];
  const as = (latest.assigneeBag || [])[0];
  return (as && (as.assigneeNameText || as.assigneeName) || '').trim();
}

// Current owner from the assignment chain (ODP assignment section). Empty when no
// ownership-transfer assignment is recorded.
export async function fetchAssignmentOwner(appNum, timeoutMs = 10000) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  let res;
  try {
    res = await fetch(
      `${BASE}/applications/${encodeURIComponent(appNum)}/assignment`,
      { headers: { 'X-API-KEY': getApiKey(), Accept: 'application/json' }, signal: controller.signal }
    );
  ...
  const data = await res.json().catch(() => ({}));
  const rec = (Array.isArray(data.patentFileWrapperDataBag) && data.patentFileWrapperDataBag[0]) || data;
  return pickAssignmentOwner(rec.assignmentBag || rec.patentAssignmentBag || []);
}
