...
export function classifyFwd(text) {
  ...
  // 2) Quantifier disposition — "{No|None|All|Some} challenged claim(s) … [are/is/
  // held] unpatentable" — stated in the ORDER rather than the caption. "No/none …
  // unpatentable" is an unambiguous PO win regardless of surrounding wording.
  const quant = (w) => new RegExp(`\\b${w}\\s+(?:of\\s+the\\s+)?challenged\\s+claims?\\b[^.]{0,60}\\bunpatentab`, 'i').test(t);
  const someU = quant('some');
  const noU = quant('no') || quant('none');
  const allU = quant('all');

  // 3) Negation-aware holding language. "has not shown … unpatentable" must NOT
  // read as an affirmative unpatentability holding; "held unpatentable" only
  // counts affirmatively when not preceded by "not".
