...
// Store the parsed outcome (and, when supplied, cache the OCR text so future
// re-parses are instant). A null text leaves any cached text untouched.
export async function setConclusionOutcome(appNum, o, text) {
  await ensureSchema();
...
export async function getConclusionText(appNum) {
  await ensureSchema();
  const { rows } = await sql`SELECT cert_doc_id, cert_text, outcome_summary FROM reexam_conclusions WHERE application_number = ${appNum}`;
  return rows[0] || null;
}
