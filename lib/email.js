...
// Optional 6th section: PTAB final written decisions issued the prior day. Each
// row links to its /trial page, the challenged patent (Google Patents), and the
// decision PDF (via the site's proxy). ptab items: { trial, type, patent, po,
// petitioner, pdfUrl }.
function ptabSection(ptab) {
  ...
  return `<div style="margin:0 0 26px"><h3 style="margin:0 0 4px;font-size:15px;color:#1a3a6b">6. PTAB Final Written Decisions</h3><p style="color:#718096;font-size:12px;margin:0 0 6px">Outcome is not yet classified on the day of issuance — open the trial page for the parsed result.</p>${table}</div>`;
}
...
export async function sendComprehensiveDigestTo(email, events, opts = {}) {
  const { dateLabel = '', unsubscribeUrl = '', ptabDecisions = [] } = opts;
  const base = baseUrl();
  const total = events.length + ptabDecisions.length;
  const label = dateLabel ? ` (${dateLabel})` : '';
  const body = comprehensiveSections(events) + ptabSection(ptabDecisions);
  const html = `<div style="font-family:-apple-system,Segoe UI,Roboto,sans-serif;max-width:720px;margin:0 auto;color:#2d3748">
    ...
    <p style="color:#718096;font-size:14px">Relevant ex parte reexamination filings issued the previous day (by category), plus any new PTAB final written decisions.</p>
