  ...
  return `<table style="border-collapse:collapse;width:100%;background:#fff">
    <thead><tr>
      ...
      return `<tr>
        ...
        <td style="padding:6px 10px;border-top:1px solid #e2e8f0;font-size:13px">${esc(e.label || e.doc_code || '')}</td>
...
// Document cell for the PTAB tables. The reexam rows have offered View / Download
// since the start while these offered only a single "View", so the same action was
// available or not depending on which section a document happened to fall in.
// ?dl=1 is what the /api/ptab proxy uses to switch to an attachment, and ?name=
// gives the download a filename better than the proxy's generic default.
function ptabFileLinks(pdfUrl, trial) {
  const base = baseUrl();
  if (!base || !pdfUrl) return '&mdash;';
  const name = String(trial || 'decision').replace(/[^A-Za-z0-9._-]/g, '');
  const u = (dl) => `${base}/api/ptab?file=${encodeURIComponent(pdfUrl)}&name=${encodeURIComponent(name)}${dl ? '&dl=1' : ''}`;
  return `<a href="${u(false)}" style="color:#1a3a6b;font-weight:600;text-decoration:none">View</a>`
       + ` / `
       + `<a href="${u(true)}" style="color:#1a3a6b;font-weight:600;text-decoration:none">Download</a>`;
}
...
function ptabDecisionsPart(items) {
  ...
  const rows = items.map((p) => {
    ...
    const viewCell = ptabFileLinks(p.pdfUrl, p.trial);
  ...
  const table = `<table style="border-collapse:collapse;width:100%;background:#fff"><thead><tr>${th('Trial No.')}${th('Type')}${th('Patent')}${th('Patent Owner')}${th('Decision')}${th('View / Download')}</tr></thead><tbody>${rows}</tbody></table>`;
...
function ptabFwdPart(ptab) {
  ...
  const table = `<table style="border-collapse:collapse;width:100%;background:#fff"><thead><tr>${th('Trial No.')}${th('Type')}${th('Patent')}${th('Patent Owner')}${th('Petitioner')}${th('View / Download')}</tr></thead><tbody>${rows}</tbody></table>`;
