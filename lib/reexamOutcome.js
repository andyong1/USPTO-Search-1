...
// True if the text references a substantial new question of patentability (SNQ),
// used to flag pre-order petition decisions that discuss the SNQ standard.
// Best-effort: only works on machine-readable text.
export function detectSnq(text) {
  if (!text) return false;
  return /substantial\s+new\s+question/i.test(text) || /\bSNQ\b/.test(text);
}
