...
const ITC_SCHEMA_VERSION = '2026-07-29.3';
...
async function migrate() {
  ...
  // Added after itc_investigation shipped — ALTER for existing DBs (analytics: pendency + by-ALJ).
  await sql`ALTER TABLE itc_investigation ADD COLUMN IF NOT EXISTS decision_date text`;  // latest dispositive-doc date
  await sql`ALTER TABLE itc_investigation ADD COLUMN IF NOT EXISTS alj text`;             // presiding ALJ (partial coverage)
...
export async function listInvestigations() {
  ...
  const { rows } = await sql`
    SELECT investigation_number, investigation_phase, public_number, title, status,
           docket_number, institution_date, last_doc_date, decision_date, alj, doc_count, public_doc_count,
...
export async function investigationsToExtract(limit, number, onlyNeedy) {
  await ensureItcSchema();
  const n = number || null;
  const needy = onlyNeedy ? true : false;
  const { rows } = await sql`
    ...
    WHERE (${n}::text IS NULL OR investigation_number = ${n})
      ...
      AND (${needy} = false OR investigation_number NOT IN (SELECT investigation_number FROM itc_doc_text WHERE coalesce(text, '') <> ''))
...
// Direct-download links to the key outcome documents, for the outcomes column.
// Per investigation, the most-recent MIRRORED (public R2/Blob copy) exclusion
// order, cease-and-desist order, and Commission opinion — so a GEO/LEO or CDO
// chip can link straight to the order PDF and the outcome badge can link to the
// opinion. Non-mirrored docs are omitted (they get no link). Keyed by number.
export async function listOutcomeDocLinks() {
  await ensureItcSchema();
  const { rows } = await sql`
    SELECT investigation_number,
      (array_agg(mirror_url ORDER BY received_date DESC NULLS LAST)
        FILTER (WHERE document_title ~* 'exclusion order'))[1]   AS excl_url,
      (array_agg(mirror_url ORDER BY received_date DESC NULLS LAST)
        FILTER (WHERE document_title ~* 'cease and desist'))[1]  AS cdo_url,
      (array_agg(mirror_url ORDER BY received_date DESC NULLS LAST)
        FILTER (WHERE document_type IN ('Opinion, Commission', 'Opinion', 'Court Opinion')))[1] AS opinion_url
    FROM itc_document
    WHERE coalesce(mirror_url, '') LIKE 'http%'
      AND (document_title ~* 'exclusion order|cease and desist'
           OR document_type IN ('Opinion, Commission', 'Opinion', 'Court Opinion'))
    GROUP BY investigation_number`;
  return rows;
}
...
// The per-run signatures that decide which detail blobs need rewriting. This
// used to live in itc-work/.publish-state.json, which is gitignored and local:
// losing that directory — or running the nightly from another machine — read as
// "no prior state" and rewrote all ~1,350 detail blobs. At one Vercel Blob
// advanced operation per write that is 68% of a Hobby month's allowance for a
// run that should have written nothing, so the state now lives beside the data
// it describes.
const PUBLISH_STATE_KEY = 'itc_publish_state';

export async function getPublishState() {
  await ensureItcSchema();
  const { rows } = await sql`SELECT v FROM ptab_kv WHERE k = ${PUBLISH_STATE_KEY} LIMIT 1`;
  if (!rows[0] || !rows[0].v) return null;
  try { return JSON.parse(rows[0].v); } catch { return null; }
}

export async function setPublishState(state) {
  await ensureItcSchema();
  await sql`INSERT INTO ptab_kv (k, v, updated_at) VALUES (${PUBLISH_STATE_KEY}, ${JSON.stringify(state || {})}, now())
