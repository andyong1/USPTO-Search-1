...
function threadOneFamily(rows) {
  ...
  for (const r of sorted) {
    if (r.kind === 'petition') {
      ...
      // The latest open thread is also the nearest in time, so if its gap is
      // already too wide no earlier thread can be a better fit.
      const t = [...open].reverse().find((x) => !x.opposition);
      const gap = t ? dayGap(t.petition && t.petition.official_date, r.official_date) : null;
      if (t && (gap === null || gap <= OPPOSITION_MAX_GAP_DAYS)) t.opposition = r;
