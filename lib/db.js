...
async function runMigrations() {
  ...
  // Monthly filing counts for the (unlinked) filings-trends page: kind =
  // 'reexam' (ex parte 90/* applications) or 'ipr' (IPR petitions), ym = 'YYYY-MM'.
  await sql`CREATE TABLE IF NOT EXISTS filings_monthly (kind text, ym text, count int, updated_at timestamptz DEFAULT now(), PRIMARY KEY (kind, ym))`;
...
export async function upsertPtabFwdMeta(r) {
  ...
  await sql`
    ...
    ON CONFLICT (trial_number) DO UPDATE SET
      ...
      -- When the FWD advances to a newer decision, re-extract + reclassify it.
      extracted_v  = CASE WHEN EXCLUDED.fwd_date > coalesce(ptab_fwd.fwd_date, '') THEN 0 ELSE ptab_fwd.extracted_v  END,
      classified_v = CASE WHEN EXCLUDED.fwd_date > coalesce(ptab_fwd.fwd_date, '') THEN 0 ELSE ptab_fwd.classified_v END`;
}
...
// ── Monthly filing counts (filings-trends page) ─────────────────────
export async function upsertFilingCount(kind, ym, count) {
  await ensureSchema();
  await sql`INSERT INTO filings_monthly (kind, ym, count, updated_at) VALUES (${kind}, ${ym}, ${count}, now())
            ON CONFLICT (kind, ym) DO UPDATE SET count = ${count}, updated_at = now()`;
}
export async function listFilings() {
  await ensureSchema();
  const { rows } = await sql`SELECT kind, ym, count, updated_at FROM filings_monthly ORDER BY ym`;
  return rows;
}
