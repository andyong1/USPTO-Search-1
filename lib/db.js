...
const SCHEMA_VERSION = '2026-08-12.1';
...
async function runMigrations() {
  ...
  // Outcome of the most recent digest attempt. Without this a failed send left no
  // trace anywhere: the only record was the HTTP response body, which the
  // scheduler discards, so a digest that silently failed to send looked exactly
  // like a quiet day with nothing to report.
  await sql`ALTER TABLE reexam_state ADD COLUMN IF NOT EXISTS last_sub_digest_result jsonb`;
  ...
  // Unrecognized-but-petition-looking document codes, recorded as the petition
  // harvest walks wrappers it already fetched (zero extra API calls). Exists
  // because the classifier's code list has twice proven incomplete -- RXRPET
  // ("Petition for Review of Reexam Denial") and RXRQ/T ("Reexam Request for
  // Extension of Time") were each silently dropped for months and surfaced only
  // via a client report. Anything landing here is a candidate for
  // classifyPetitionDoc; the point is that the NEXT such code announces itself.
  await sql`CREATE TABLE IF NOT EXISTS petition_code_audit (
  ...
  // Law-firm attribution per proceeding, both sides, read from the determination's
  // PTOL-90A cover sheet (see lib/firms.js). One row per proceeding, keyed on the
  // EARLIEST determination -- that is the one that answers the request, so it is
  // what a requester-side grant rate should be measured against.
  //
  // IMPORTANT semantics: these are the firms of record AT DETERMINATION TIME, not
  // today. Powers of attorney change (verified: 90015330 shows Abel Schillinger on
  // its Aug-2025 order while ODP now reports BakerHostetler), and point-in-time is
  // the correct basis for "who handled this determination".
  await sql`CREATE TABLE IF NOT EXISTS reexam_firm (
    application_number  text PRIMARY KEY,
    determination_date  text,
    determination_kind  text,   -- 'order' | 'denial'
    owner_firm_raw      text,
    owner_firm          text,
    owner_firm_key      text,
    owner_filer_type    text,
    requester_firm_raw  text,
    requester_firm      text,
    requester_firm_key  text,
    requester_filer_type text,
    firm_v              int,
    extracted_at        timestamptz NOT NULL DEFAULT now()
  )`;
  await sql`CREATE INDEX IF NOT EXISTS reexam_firm_req_key ON reexam_firm (requester_firm_key)`;
  await sql`CREATE INDEX IF NOT EXISTS reexam_firm_own_key ON reexam_firm (owner_firm_key)`;
  ...
  // AI requester-attribution pass (nightly, local): RXPET./RXPTD* codes are
  // party-agnostic, so the heuristic can flag a patent-owner petition (or the
  // decision on one) as the requester's. The AI reads the candidate documents'
  // opening pages and SELECTS the true requester petition + its decision (nulls
  // when only patent-owner papers exist). listPreorder overlays these when
  // preorder_ai_v is set; updatePreorderPetition resets the gate when the
  // heuristic picks change (e.g. a new decision posts) so the row re-verifies.
  await sql`ALTER TABLE reexam_preorder ADD COLUMN IF NOT EXISTS ai_pet_doc_id text`;
  await sql`ALTER TABLE reexam_preorder ADD COLUMN IF NOT EXISTS ai_pet_date text`;
  await sql`ALTER TABLE reexam_preorder ADD COLUMN IF NOT EXISTS ai_dec_doc_id text`;
  await sql`ALTER TABLE reexam_preorder ADD COLUMN IF NOT EXISTS ai_dec_date text`;
  await sql`ALTER TABLE reexam_preorder ADD COLUMN IF NOT EXISTS ai_dec_outcome text`;
  await sql`ALTER TABLE reexam_preorder ADD COLUMN IF NOT EXISTS ai_confidence text`;
  await sql`ALTER TABLE reexam_preorder ADD COLUMN IF NOT EXISTS ai_note text`;
  await sql`ALTER TABLE reexam_preorder ADD COLUMN IF NOT EXISTS preorder_ai_v int`;
  // Cached first-pages text of petition-cluster candidate documents, keyed by
  // (proceeding, document). Shared by the pre-order AI pass (preorder-fetch.mjs)
  // and the post-order AI pass (postpet-fetch.mjs), so AI-version bumps re-read
  // without re-downloading.
  await sql`CREATE TABLE IF NOT EXISTS preorder_doc_text (
    application_number text NOT NULL,
    ...
    PRIMARY KEY (application_number, doc_id)
  )`;
  ...
  // AI-verified claim disposition (read from cert_text by the nightly local Claude
  // session — more accurate than the regex parse on messy certificate OCR). Vercel
  // only serves these; cert_ai_v gates the resumable verify pass.
  await sql`ALTER TABLE reexam_conclusions ADD COLUMN IF NOT EXISTS ai_confirmed text`;
  await sql`ALTER TABLE reexam_conclusions ADD COLUMN IF NOT EXISTS ai_cancelled text`;
  await sql`ALTER TABLE reexam_conclusions ADD COLUMN IF NOT EXISTS ai_amended text`;
  await sql`ALTER TABLE reexam_conclusions ADD COLUMN IF NOT EXISTS ai_new text`;
  await sql`ALTER TABLE reexam_conclusions ADD COLUMN IF NOT EXISTS ai_summary text`;
  await sql`ALTER TABLE reexam_conclusions ADD COLUMN IF NOT EXISTS ai_confidence text`;
  await sql`ALTER TABLE reexam_conclusions ADD COLUMN IF NOT EXISTS ai_note text`;
  await sql`ALTER TABLE reexam_conclusions ADD COLUMN IF NOT EXISTS cert_ai_v int DEFAULT 0`;
  ...
  // Patent owner (the underlying application's applicant of record — firstApplicantName
  // from ODP meta-data). owner_v gates a one-time backfill of existing rows.
  await sql`ALTER TABLE reexam_tech_center ADD COLUMN IF NOT EXISTS patent_owner text`;
  await sql`ALTER TABLE reexam_tech_center ADD COLUMN IF NOT EXISTS owner_v int DEFAULT 0`;
  ...
  // AI attribution pass (nightly, local — same pattern as the pre-order page):
  // PET.OP/RXPET. are party- and purpose-agnostic, so the timing heuristic can
  // flag a requester paper or a non-325(d) PO petition, and with several
  // petitions pending the earliest decision can rule on the wrong one. The AI
  // reads candidates' opening pages and SELECTS the PO 325(d) petition, the
  // requester opposition, and the decision that rules on THAT petition.
  await sql`ALTER TABLE reexam_post_petitions ADD COLUMN IF NOT EXISTS ai_pet_doc_id text`;
  await sql`ALTER TABLE reexam_post_petitions ADD COLUMN IF NOT EXISTS ai_pet_date text`;
  await sql`ALTER TABLE reexam_post_petitions ADD COLUMN IF NOT EXISTS ai_pet_325d boolean`;
  await sql`ALTER TABLE reexam_post_petitions ADD COLUMN IF NOT EXISTS ai_opp_doc_id text`;
  await sql`ALTER TABLE reexam_post_petitions ADD COLUMN IF NOT EXISTS ai_opp_date text`;
  await sql`ALTER TABLE reexam_post_petitions ADD COLUMN IF NOT EXISTS ai_dec_doc_id text`;
  await sql`ALTER TABLE reexam_post_petitions ADD COLUMN IF NOT EXISTS ai_dec_date text`;
  await sql`ALTER TABLE reexam_post_petitions ADD COLUMN IF NOT EXISTS ai_dec_outcome text`;
  await sql`ALTER TABLE reexam_post_petitions ADD COLUMN IF NOT EXISTS ai_dec_325d boolean`;
  await sql`ALTER TABLE reexam_post_petitions ADD COLUMN IF NOT EXISTS ai_confidence text`;
  await sql`ALTER TABLE reexam_post_petitions ADD COLUMN IF NOT EXISTS ai_note text`;
  await sql`ALTER TABLE reexam_post_petitions ADD COLUMN IF NOT EXISTS post_ai_v int`;
  ...
  // NIRC prior-art comparison (nightly AI pass — /reexam-nirc). Compares the art
  // underlying the request's SNQs, as recited in the reexam ORDER (already stored
  // in reexam_doc_text, doc_kind='order'), against the art discussed in the Notice
  // of Intent to Issue a Reexam Certificate (NIRC) — tagged by role (invalidating
  // / distinguished / mentioned) — to measure how often the request's art is
  // operative at conclusion. Prose extraction + cross-document reference matching
  // is AI-only; the percentage is computed in code from these stored lists so it
  // is reproducible. nirc_text caches the fetched/OCR'd NIRC so version bumps
  // re-read without re-downloading.
  await sql`CREATE TABLE IF NOT EXISTS reexam_nirc_art (
    application_number text PRIMARY KEY,
    nirc_doc_id        text,
    nirc_date          text,
    nirc_text          text,
    req_refs           jsonb,   -- [{label, key}] art in the order's SNQs
    nirc_refs          jsonb,   -- [{label, key, role}] art discussed in the NIRC
    matches            jsonb,   -- [{label, key, role}] request refs operative in the NIRC
    req_count          int,
    nirc_count         int,
    match_count        int,
    match_invalidating int,
    match_distinguished int,
    basis              text,    -- reasons-stated | as-amended | not-stated | no-nirc-art
    ai_confidence      text,
    ai_note            text,
    nirc_ai_v          int,
    updated_at         timestamptz NOT NULL DEFAULT now()
  )`;
  // "Final Office action" bucket: when the NIRC itself states no art-based
  // reasons (basis not-stated/no-nirc-art), the operative art lives in the last
  // substantial Office action (RXFR.. final rejection, else the latest RXR.NF
  // non-final). A second AI pass reads that action and fills nirc_refs/matches
  // from it; art_source records provenance ('nirc' when from the NIRC itself,
  // 'final-oa' when from the action). oa_text caches the fetched action.
  await sql`ALTER TABLE reexam_nirc_art ADD COLUMN IF NOT EXISTS art_source text`;
  await sql`ALTER TABLE reexam_nirc_art ADD COLUMN IF NOT EXISTS oa_doc_id text`;
  await sql`ALTER TABLE reexam_nirc_art ADD COLUMN IF NOT EXISTS oa_date text`;
  await sql`ALTER TABLE reexam_nirc_art ADD COLUMN IF NOT EXISTS oa_code text`;
  await sql`ALTER TABLE reexam_nirc_art ADD COLUMN IF NOT EXISTS oa_text text`;
  await sql`ALTER TABLE reexam_nirc_art ADD COLUMN IF NOT EXISTS oa_ai_v int`;
  // Count of Office actions (RXFR.. final + RXR.NF non-final) issued before the
  // NIRC — deterministic doc-code count, populated by oacount.mjs. NULL = not
  // yet counted; 0 = confirmed none.
  await sql`ALTER TABLE reexam_nirc_art ADD COLUMN IF NOT EXISTS oa_count int`;
  ...
  // AI verification of the FWD outcome (read from decision_text by the nightly
  // local Claude session — independent second read to measure/correct the regex
  // classifier and add claim-level detail for partials). Stored SEPARATELY from
  // `outcome` (which the win-rate stats use) until the disagreement rate is
  // reviewed. fwd_ai_v gates the resumable verify pass.
  await sql`ALTER TABLE ptab_fwd ADD COLUMN IF NOT EXISTS ai_outcome text`;
  await sql`ALTER TABLE ptab_fwd ADD COLUMN IF NOT EXISTS ai_unpatentable text`;
  await sql`ALTER TABLE ptab_fwd ADD COLUMN IF NOT EXISTS ai_upheld text`;
  await sql`ALTER TABLE ptab_fwd ADD COLUMN IF NOT EXISTS ai_confidence text`;
  await sql`ALTER TABLE ptab_fwd ADD COLUMN IF NOT EXISTS ai_note text`;
  await sql`ALTER TABLE ptab_fwd ADD COLUMN IF NOT EXISTS fwd_ai_v int DEFAULT 0`;
  ...
  // Patent issue (grant) date of the challenged patent, for the "enforcement age"
  // (issue date -> IPR petition date) column. '' = fetched, none found. Populated
  // by ?issuedates=1 (one metadata fetch per decision's application).
  await sql`ALTER TABLE ptab_decisions ADD COLUMN IF NOT EXISTS patent_issue_date text`;
  ...
  // AI-generated §325(d) summary (written by the nightly local pipeline — the
  // scheduled Claude session reads the stored OCR text and uploads the result;
  // Vercel only serves it). d325_sum_v gates the resumable backfill.
  await sql`ALTER TABLE reexam_doc_text ADD COLUMN IF NOT EXISTS d325_addressed text`;
  await sql`ALTER TABLE reexam_doc_text ADD COLUMN IF NOT EXISTS d325_summary text`;
  await sql`ALTER TABLE reexam_doc_text ADD COLUMN IF NOT EXISTS d325_sum_v int DEFAULT 0`;
  // Related district-court litigation for the reexamined patent, read from the
  // examiner's RXLITSR ("Reexam Litigation Search Conducted") document — an
  // image-only scan, OCR'd and AI-extracted by the rxlit-* pipeline. cases is a
  // jsonb array of {caption, case_no, court, status}; none_found records an
  // explicit "NO LITIGATION WAS FOUND". Court is enriched from the determination
  // text where the search report gives only a docket number. rxlit_v gates re-runs
  // (a row with rxlit_v set but litsr_doc_id NULL means the reexam has no RXLITSR).
  await sql`CREATE TABLE IF NOT EXISTS reexam_litigation (
    application_number text PRIMARY KEY,
    cases              jsonb,
    none_found         boolean,
    litsr_doc_id       text,
    ...
    ai_confidence      text,
    ai_note            text,
    rxlit_v            int DEFAULT 0,
    updated_at         timestamptz NOT NULL DEFAULT now()
  )`;
  // Authoritative third-party requester identity, extracted from the reexam REQUEST
  // document (doc code RXOSUB.R*, "Receipt of Original Ex Parte Request by Third
  // Party") — the opening names the requester as X ("Requester"). Image-only scans,
  // OCR'd + AI-extracted by the reqid-* pipeline. reqid_v gates re-runs; a row with
  // reqid_v set but req_doc_id NULL means no request document was found.
  await sql`CREATE TABLE IF NOT EXISTS reexam_requester (
    application_number text PRIMARY KEY,
    requester_name     text,
    req_doc_id         text,
    req_date           text,
    ...
    ai_confidence      text,
    ai_note            text,
    reqid_v            int DEFAULT 0,
    updated_at         timestamptz NOT NULL DEFAULT now()
  )`;
  // Every petition-related DOCUMENT (filing / opposition / decision) seen in a
  // tracked reexam's file wrapper, one row per document. Harvested for free from
  // the certificate-check scan (same docs feed) plus a one-time backfill sweep.
  // Rows are immutable facts; petition→opposition→decision THREADING is done at
  // read time (threadPetitions in lib/petitions.js), so no pairing to migrate.
  await sql`CREATE TABLE IF NOT EXISTS reexam_petition_docs (
    doc_id             text PRIMARY KEY,
    application_number text NOT NULL,
    official_date      text,
    doc_code           text,
    kind               text,   -- 'petition' | 'opposition' | 'decision'
    outcome            text,   -- decisions only: granted | dismissed | denied | decided
    found_at           timestamptz NOT NULL DEFAULT now()
  )`;
  await sql`CREATE INDEX IF NOT EXISTS reexam_petition_docs_app ON reexam_petition_docs (application_number)`;
  // Subject matter of each petition DECISION, from an OCR + AI pass over the
  // decision text (the decision recites the relief the petition requested —
  // validated 29/29 on a sample). Relief is MULTI-LABEL because these decisions
  // routinely dispose of several asks at once (typically a 37 CFR 1.183 waiver
  // granted so the Office can reach a § 325(d) termination request, which is then
  // dismissed). primary_relief/merits_outcome capture the SUBSTANTIVE ask and its
  // disposition so statistics aren't inflated by ancillary procedural grants;
  // ancillary_waiver records the procedural leg separately.
  await sql`CREATE TABLE IF NOT EXISTS reexam_petition_subject (
    doc_id             text PRIMARY KEY,
    application_number text NOT NULL,
    reliefs            text[],   -- every relief requested (controlled vocabulary)
    primary_relief     text,     -- the merits / ultimate relief sought
    merits_outcome     text,     -- granted | granted_in_part | dismissed | denied | undecided | other
    ancillary_waiver   text,     -- granted | dismissed | denied | null (the procedural leg, if any)
    -- Which relief the Office actually granted, when that differs from the
    -- petitioner's ultimate aim. In the dominant pattern the Office grants leave
    -- (a 1.183 waiver) to file a § 325(d) paper and refers the § 325(d) question
    -- itself to the CRU, which answers it in the reexamination determination —
    -- there is never a second petition decision. So the petition IS granted, and
    -- referred_to_cru records where the substantive question went.
    granted_relief     text,
    referred_to_cru    boolean DEFAULT false,
    rules              text[],
    statutes           text[],
    petitioner         text,     -- patent_owner | third_party_requester | unclear
    relief_verbatim    text,
    ai_confidence      text,
    ai_note            text,
    petsubj_v          int DEFAULT 0,
    updated_at         timestamptz NOT NULL DEFAULT now()
  )`;
  await sql`ALTER TABLE reexam_petition_subject ADD COLUMN IF NOT EXISTS granted_relief text`;
  await sql`ALTER TABLE reexam_petition_subject ADD COLUMN IF NOT EXISTS referred_to_cru boolean DEFAULT false`;
  await sql`CREATE INDEX IF NOT EXISTS reexam_petition_subject_app ON reexam_petition_subject (application_number)`;
  // Relief AS FILED, read from the PETITION document itself (front pages) rather
  // than from the Office's later decision. Kept in its own table because the two
  // answer different questions: reexam_petition_subject records how the Office
  // CHARACTERIZED the request (and pairs with the outcome), while this records
  // what the petitioner ASKED FOR. It also supplies relief for petitions with no
  // decision yet — the majority of rows on the page. No outcome column: a
  // petition has no disposition.
  await sql`CREATE TABLE IF NOT EXISTS reexam_petition_request (
    doc_id             text PRIMARY KEY,
    application_number text NOT NULL,
    reliefs            text[],
    primary_relief     text,
    ...
    ai_confidence      text,
    ai_note            text,
    -- Reading the papers revealed that ~20% of documents filed under petition
    -- codes are not petitions at all: exhibits to a petition (patent copies,
    -- prior-art publications, declarations, docket sheets), standalone
    -- oppositions, or Office papers. They were inflating the petition count and
    -- creating phantom threads, so they're flagged here and excluded downstream.
    is_petition        boolean DEFAULT true,
    petreq_v           int DEFAULT 0,
    updated_at         timestamptz NOT NULL DEFAULT now()
  )`;
  await sql`ALTER TABLE reexam_petition_request ADD COLUMN IF NOT EXISTS is_petition boolean DEFAULT true`;
  await sql`CREATE INDEX IF NOT EXISTS reexam_petition_request_app ON reexam_petition_request (application_number)`;
  // What an OPPOSITION is answering. Pairing an opposition to a petition by
  // filing order alone is guesswork, and it guessed wrong: in 90/015,704 a paper
  // opposing a patent owner filing the Office never entered into the wrapper was
  // attached to an unrelated requester petition. An opposition names its target
  // in the caption ("Opposition to Patent Owner's Petition filed March 30,
  // 2026"), so reading it turns the pairing into evidence. `opposes_date` is
  // stored as the AI read it -- a free-text date, not a foreign key -- because
  // the paper it names may not exist in our data at all, which is exactly the
  // finding that matters.
  await sql`CREATE TABLE IF NOT EXISTS reexam_petition_opposition (
    doc_id             text PRIMARY KEY,
    application_number text NOT NULL,
    party              text,   -- patent_owner | third_party_requester | unclear
    opposes_date       text,   -- ISO date of the paper opposed, as stated; null if unstated
    opposes_verbatim   text,
    -- Same problem as the petition codes: not everything filed under an
    -- opposition code is an opposition.
    is_opposition      boolean DEFAULT true,
    ai_confidence      text,
    ai_note            text,
    petopp_v           int DEFAULT 0,
    updated_at         timestamptz NOT NULL DEFAULT now()
  )`;
  await sql`CREATE INDEX IF NOT EXISTS reexam_petition_opposition_app ON reexam_petition_opposition (application_number)`;
  ...
  // AI residual litigation pass: for rows the regex left empty, the nightly Claude
  // session reads the stored front-matter and extracts courts it missed (prose
  // formats). Stored separately; getLitigationMap prefers the regex arrays and
  // only falls back to these when the regex found nothing. lit_ai_v gates it.
  await sql`ALTER TABLE ptab_petition_refs ADD COLUMN IF NOT EXISTS ai_lit_petitioner text[]`;
  await sql`ALTER TABLE ptab_petition_refs ADD COLUMN IF NOT EXISTS ai_lit_other text[]`;
  await sql`ALTER TABLE ptab_petition_refs ADD COLUMN IF NOT EXISTS lit_ai_note text`;
  await sql`ALTER TABLE ptab_petition_refs ADD COLUMN IF NOT EXISTS lit_ai_v int DEFAULT 0`;
}
  ...
  const { rows } = await sql`
    SELECT * FROM (
      SELECT DISTINCT ON (p.application_number)
             ...
             -- AI requester-attribution overlay: once verified (preorder_ai_v set),
             -- the AI's picks replace the code-heuristic ones — including nulls,
             -- which blank the cells when only patent-owner papers exist.
             CASE WHEN p.preorder_ai_v IS NOT NULL THEN p.ai_pet_doc_id  ELSE p.petition_doc_id  END AS petition_doc_id,
             CASE WHEN p.preorder_ai_v IS NOT NULL THEN p.ai_pet_date    ELSE p.petition_date    END AS petition_date,
             CASE WHEN p.preorder_ai_v IS NOT NULL THEN p.ai_dec_doc_id  ELSE p.decision_doc_id  END AS decision_doc_id,
             CASE WHEN p.preorder_ai_v IS NOT NULL THEN p.ai_dec_date    ELSE p.decision_date    END AS decision_date,
             CASE WHEN p.preorder_ai_v IS NOT NULL THEN p.ai_dec_outcome ELSE p.decision_outcome END AS decision_outcome,
...
// When the heuristic's picks CHANGE (a new petition/decision posted), clear the
// AI-verification gate so the nightly pass re-reads the documents; an unchanged
// re-run leaves a completed verification in place.
export async function updatePreorderPetition(appNum, petition, decision) {
  await ensureSchema();
  await sql`UPDATE reexam_preorder SET
      preorder_ai_v = CASE WHEN petition_doc_id IS DISTINCT FROM ${petition ? petition.id : null}
                             OR decision_doc_id IS DISTINCT FROM ${decision ? decision.id : null}
                           THEN NULL ELSE preorder_ai_v END,
...
// ── AI requester-attribution pass for pre-order petitions ──────────
// Version of the verification prompt/logic; bump to re-verify every row.
const PREORDER_AI_V = 1;
export async function setPreorderAi(appNum, o) {
  await sql`UPDATE reexam_preorder SET
      ai_pet_doc_id  = ${o.petDocId || null},  ai_pet_date  = ${o.petDate || null},
      ai_dec_doc_id  = ${o.decDocId || null},  ai_dec_date  = ${o.decDate || null},
      ai_dec_outcome = ${o.decOutcome || null},
      ai_confidence  = ${o.confidence || null}, ai_note = ${o.note || null},
      preorder_ai_v  = ${PREORDER_AI_V}
    WHERE application_number = ${appNum}`;
}

// ── AI attribution pass for POST-order patent owner petitions ──────
// Version of the verification prompt/logic; bump to re-verify every row.
const POST_AI_V = 1;
export async function setPostPetAi(appNum, o) {
  await sql`UPDATE reexam_post_petitions SET
      ai_pet_doc_id = ${o.petDocId || null}, ai_pet_date = ${o.petDate || null},
      ai_pet_325d   = ${o.pet325d === undefined ? null : o.pet325d},
      ai_opp_doc_id = ${o.oppDocId || null}, ai_opp_date = ${o.oppDate || null},
      ai_dec_doc_id = ${o.decDocId || null}, ai_dec_date = ${o.decDate || null},
      ai_dec_outcome = ${o.decOutcome || null},
      ai_dec_325d   = ${o.dec325d === undefined ? null : o.dec325d},
      ai_confidence = ${o.confidence || null}, ai_note = ${o.note || null},
      post_ai_v     = ${POST_AI_V}
    WHERE application_number = ${appNum}`;
}
...
// ── AI FWD-outcome verification (nightly local Claude pass) ──
// Bump to re-verify every FWD (independent of the regex CLASSIFIER_V).
const FWD_AI_V = 1;
// Store the AI-verified outcome. o = { outcome, unpatentable, upheld, confidence, note }.
export async function setFwdAiOutcome(trial, o) {
  await sql`
    UPDATE ptab_fwd SET
      ai_outcome = ${o.outcome || null}, ai_unpatentable = ${o.unpatentable || null},
      ai_upheld = ${o.upheld || null}, ai_confidence = ${o.confidence || null},
      ai_note = ${o.note || null}, fwd_ai_v = ${FWD_AI_V}
    WHERE trial_number = ${String(trial)}`;
}
...
export async function getPtabFwdToClassify(limit, currentV) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT trial_number, decision_text, ai_outcome, ai_confidence FROM ptab_fwd
    WHERE (classified_v IS NULL OR classified_v < ${currentV}) AND coalesce(extracted_v, 0) > 0
    ORDER BY fwd_date DESC NULLS LAST LIMIT ${limit}`;
  return rows;
}
// The outcome to store for an FWD row: the regex classification, but overridden
// by a high-confidence AI verification when they disagree, and by ANY AI answer
// when the regex could not classify (needs_review/other). Keeps the fast/free
// regex as the daily classifier while letting the nightly AI pass correct its
// rare misses (measured ~0.3%) durably across CLASSIFIER_V bumps.
export function resolveFwdOutcome(row, regex) {
  const ai = row.ai_outcome, conf = row.ai_confidence;
  if (ai && conf === 'high' && ai !== regex.outcome) return { outcome: ai, detail: `AI-verified (regex: ${regex.outcome})` };
  if (ai && ai !== 'needs_review' && (regex.outcome === 'needs_review' || regex.outcome === 'other')) return { outcome: ai, detail: 'AI-verified (regex unclassified)' };
  return regex;
}
...
// v3 = ~23KB window + re-extraction on full text with the hardened extractor).
const FM_V = 3;
...
export async function getReexamGroundsMap() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT application_number, ref_numbers, trial_mentions, cites_325d, d325_level,
           d325_addressed, d325_summary, official_date
    FROM reexam_doc_text
    WHERE coalesce(grounds_v, 0) >= ${GROUNDS_V}`;
  const RANK = { none: 0, recited: 1, substantive: 2 };
  const map = new Map();
  for (const r of rows) {
    const e = map.get(r.application_number) || { refs: new Set(), trials: new Set(), cites325d: false, d325: 'none', d325Addressed: null, d325Summary: null, sumDate: '' };
    ...
    // AI summary: keep the LATEST summarized doc's take (a later denial/order
    // supersedes an earlier one's discussion).
    const dt = String(r.official_date || '');
    if (r.d325_summary && dt >= e.sumDate) { e.d325Addressed = r.d325_addressed || null; e.d325Summary = r.d325_summary; e.sumDate = dt; }
    map.set(r.application_number, e);
  }
  // Freeze to plain arrays for JSON/use downstream.
  const out = new Map();
  for (const [k, v] of map) out.set(k, { refs: [...v.refs], trials: [...v.trials], cites325d: v.cites325d, d325: v.d325, d325Addressed: v.d325Addressed, d325Summary: v.d325Summary });
  return out;
}
// AI §325(d) summaries for specific determination docs (digest enrichment).
export async function getD325SummariesByDocIds(docIds) {
  const out = new Map();
  if (!docIds || !docIds.length) return out;
  await ensureSchema();
  const { rows } = await sql`
    SELECT doc_id, d325_addressed, d325_summary FROM reexam_doc_text
    WHERE doc_id = ANY(${docIds}) AND coalesce(d325_summary, '') <> ''`;
  for (const r of rows) out.set(r.doc_id, { addressed: r.d325_addressed || null, summary: r.d325_summary });
  return out;
}
...
// Prefers the regex extraction; falls back to the AI residual pass only when the
// regex found NOTHING (the AI pass only ran on those fully-empty rows).
export async function getLitigationMap() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT trial_number, lit_petitioner, lit_other, ai_lit_petitioner, ai_lit_other FROM ptab_petition_refs
    WHERE coalesce(grounds_v, 0) >= ${GROUNDS_V}
      AND (lit_petitioner IS NOT NULL OR lit_other IS NOT NULL OR ai_lit_petitioner IS NOT NULL OR ai_lit_other IS NOT NULL)`;
  const map = new Map();
  for (const r of rows) {
    const hasRegex = (r.lit_petitioner || []).length + (r.lit_other || []).length > 0;
    map.set(r.trial_number, hasRegex
      ? { petitioner: r.lit_petitioner || [], other: r.lit_other || [] }
      : { petitioner: r.ai_lit_petitioner || [], other: r.ai_lit_other || [] });
  }
  return map;
}
// ── AI residual litigation pass (nightly local Claude) ──
const LIT_AI_V = 1;
export async function setLitAi(trial, o) {
  await sql`
    UPDATE ptab_petition_refs SET
      ai_lit_petitioner = ${o.petitioner || []}, ai_lit_other = ${o.other || []},
      lit_ai_note = ${o.note || null}, lit_ai_v = ${LIT_AI_V}
    WHERE trial_number = ${String(trial)}`;
}
...
// Decisions still missing the challenged patent's issue date, for ?issuedates=1.
// &retry re-pools ones fetched empty (no grant date found last time).
export async function getDecisionsMissingIssueDate(limit, retry) {
  await ensureSchema();
  if (retry) await sql`UPDATE ptab_decisions SET patent_issue_date = NULL WHERE patent_issue_date = ''`;
  const { rows } = await sql`
    SELECT trial_number, application_number FROM ptab_decisions
    WHERE patent_issue_date IS NULL AND coalesce(application_number, '') <> ''
    LIMIT ${limit}`;
  return rows;
}
export async function setIssueDate(trialNumber, issueDate) {
  await sql`UPDATE ptab_decisions SET patent_issue_date = ${issueDate || ''} WHERE trial_number = ${trialNumber}`;
}
export async function countDecisionsMissingIssueDate() {
  await ensureSchema();
  const { rows } = await sql`SELECT count(*)::int AS n FROM ptab_decisions WHERE patent_issue_date IS NULL AND coalesce(application_number,'') <> ''`;
  return rows[0] ? rows[0].n : 0;
}
...
// Atomically increment an integer counter stored in ptab_kv; returns the new value.
export async function bumpPtabKv(k) {
  await ensureSchema();
  const { rows } = await sql`
    INSERT INTO ptab_kv (k, v, updated_at) VALUES (${k}, '1', now())
    ON CONFLICT (k) DO UPDATE SET v = (coalesce(NULLIF(ptab_kv.v, ''), '0')::bigint + 1)::text, updated_at = now()
    RETURNING v`;
  return rows[0] ? parseInt(rows[0].v, 10) : 0;
}
async function getCounters(keys) {
  await ensureSchema();
  const { rows } = await sql`SELECT k, v FROM ptab_kv WHERE k = ANY(${keys})`;
  const out = {};
  for (const k of keys) out[k] = 0;
  for (const r of rows) out[r.k] = parseInt(r.v, 10) || 0;
  return out;
}
...
export async function recordTechCenter(appNum, t) {
  await ensureSchema();
  await sql`
    INSERT INTO reexam_tech_center (application_number, underlying_application, underlying_patent, group_art_unit, tech_center, patent_owner, owner_v, checked_at)
    VALUES (${appNum}, ${t.underlyingApplication || null}, ${t.underlyingPatent || null}, ${t.groupArtUnit || null}, ${t.techCenter || null}, ${t.patentOwner || null}, ${OWNER_V}, now())
    ON CONFLICT (application_number) DO UPDATE SET
      ...
      patent_owner           = COALESCE(EXCLUDED.patent_owner,           reexam_tech_center.patent_owner),
      owner_v                = ${OWNER_V},
      checked_at             = now()`;
}
// Patent-owner enrichment version. Bump to re-resolve owner for every row.
// v3: prefer the CURRENT owner (latest ownership-transfer assignee); the applicant
// of record (owner at filing) is only the fallback.
const OWNER_V = 3;
// Existing tech-center rows whose patent owner hasn't been resolved at the current
// version (one-time backfill; new rows get it during detectTechCenterForApp).
export async function getDeterminationsNeedingOwner(limit) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT tc.application_number, tc.underlying_application
    FROM reexam_tech_center tc
    WHERE coalesce(tc.owner_v, 0) < ${OWNER_V}
    ORDER BY tc.application_number DESC
    LIMIT ${limit}`;
  return rows;
}
export async function countDeterminationsNeedingOwner() {
  await ensureSchema();
  const { rows } = await sql`SELECT count(*)::int AS n FROM reexam_tech_center WHERE coalesce(owner_v, 0) < ${OWNER_V}`;
  return rows[0] ? rows[0].n : 0;
}
// Store just the patent owner (backfill path); stamps owner_v so it isn't reprocessed.
export async function recordOwner(appNum, owner) {
  await ensureSchema();
  await sql`UPDATE reexam_tech_center SET patent_owner = ${owner || null}, owner_v = ${OWNER_V} WHERE application_number = ${appNum}`;
}
...
export async function recordConclusionDocs(appNum, d) {
  await ensureSchema();
  const { rows: ex } = await sql`SELECT nirc_doc_id, cert_doc_id, cert_rejected_ids FROM reexam_conclusions WHERE application_number = ${appNum}`;
  ...
  // nirc_doc_id is COALESCEd below (first one found wins and is never replaced),
  // so this is true exactly once — on the scan that first sees the NIRC.
  const nircChanged = !!d.nircDocId && d.nircDocId !== ((ex[0] && ex[0].nirc_doc_id) || null);
  ...
  await sql`
    ...
    ON CONFLICT (application_number) DO UPDATE SET
      ...
      checked_at  = now()`;
  // The NIRC is the document that announces what actually happened to the claims;
  // the certificate is the formality that follows weeks or months later. Only the
  // certificate was logged, so the digest alerted on the lagging document and was
  // silent on the leading one. Logged here so subscribers hear about the outcome
  // when it issues.
  if (nircChanged) await logDocEvent('nirc', appNum, d.nircDocId, { code: 'RXNIRC', officialDate: d.nircDate, label: 'Notice of intent to issue reexamination certificate' });
...
// ── AI-verified certificate claim outcomes (nightly local Claude pass) ──
// Version of the AI cert-verification prompt/logic; bump to force a re-verify of
// every certificate (like LIT_V for litigation).
const CERT_AI_V = 1;
// Store the AI-verified claim disposition. o = { confirmed, cancelled, amended,
// new, summary, confidence, note } (strings; empty for absent categories).
export async function setCertAiOutcome(appNum, o) {
  await sql`
    UPDATE reexam_conclusions SET
      ai_confirmed = ${o.confirmed || null}, ai_cancelled = ${o.cancelled || null},
      ai_amended = ${o.amended || null}, ai_new = ${o.new || null},
      ai_summary = ${o.summary || null}, ai_confidence = ${o.confidence || null},
      ai_note = ${o.note || null}, cert_ai_v = ${CERT_AI_V}
    WHERE application_number = ${String(appNum)}`;
}

// ── NIRC prior-art comparison (nightly AI pass) ────────────────────
const NIRC_AI_V = 1;

// ── NIRC claim-disposition fill (Outcome column) ───────────────────
// NIRC proceedings whose outcome_summary is still blank — the certificate
// hasn't issued yet, so the cert pipeline hasn't populated it. The disposition
// is stated on the NIRC's PTOL-469 cover form; a light AI pass reads it. Only
// blanks are targeted, so a later cert-derived outcome_summary is never
// overwritten and filled rows aren't re-processed.
export async function getNircOutcomesToFill(limit) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT n.application_number, c.nirc_doc_id
    FROM reexam_nirc_art n JOIN reexam_conclusions c ON c.application_number = n.application_number
    WHERE n.nirc_ai_v IS NOT NULL AND c.nirc_doc_id IS NOT NULL
      AND coalesce(c.outcome_summary, '') = ''
    ORDER BY n.application_number DESC
    LIMIT ${limit}`;
  return rows;
}
export async function countNircOutcomesToFill() {
  await ensureSchema();
  const { rows } = await sql`
...
// Fill the claim disposition from the NIRC. Only sets fields that are still
// blank (never clobbers a cert-derived outcome).
export async function setNircOutcome(appNum, o) {
  await ensureSchema();
  await sql`UPDATE reexam_conclusions SET
      outcome_summary  = coalesce(nullif(outcome_summary, ''), ${o.summary || null}),
      claims_confirmed = coalesce(nullif(claims_confirmed, ''), ${o.confirmed || null}),
      claims_cancelled = coalesce(nullif(claims_cancelled, ''), ${o.cancelled || null}),
      claims_amended   = coalesce(nullif(claims_amended, ''), ${o.amended || null}),
      claims_new       = coalesce(nullif(claims_new, ''), ${o.new || null})
    WHERE application_number = ${appNum}`;
}

// Proceedings with a NIRC and stored ORDER text, not yet AI-analyzed at the
// current version. Returns the order text (request-side art source) so the fetch
// script only needs to download the NIRC itself.
export async function getNircToAi(limit) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT c.application_number, c.nirc_doc_id, c.nirc_date, c.outcome_summary,
           o.text AS order_text, o.official_date AS order_date,
           n.nirc_text
    FROM reexam_conclusions c
    JOIN LATERAL (
      SELECT text, official_date FROM reexam_doc_text t
      WHERE t.application_number = c.application_number AND t.doc_kind = 'order'
      ORDER BY t.official_date ASC NULLS LAST LIMIT 1
    ) o ON true
    LEFT JOIN reexam_nirc_art n ON n.application_number = c.application_number
    WHERE c.nirc_doc_id IS NOT NULL AND coalesce(n.nirc_ai_v, 0) < ${NIRC_AI_V}
    ORDER BY c.nirc_date DESC NULLS LAST
    LIMIT ${limit}`;
  return rows;
}
export async function countNircToAi() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT count(*)::int AS n
    FROM reexam_conclusions c
    JOIN reexam_doc_text o ON o.application_number = c.application_number AND o.doc_kind = 'order'
    LEFT JOIN reexam_nirc_art n ON n.application_number = c.application_number
    WHERE c.nirc_doc_id IS NOT NULL AND coalesce(n.nirc_ai_v, 0) < ${NIRC_AI_V}`;
  return rows[0] ? rows[0].n : 0;
}
// Cache the fetched NIRC text (so an AI re-run at a higher version needn't
// re-download). Keyed by proceeding; leaves the AI columns untouched.
export async function cacheNircText(appNum, docId, date, text) {
  await ensureSchema();
  await sql`
    INSERT INTO reexam_nirc_art (application_number, nirc_doc_id, nirc_date, nirc_text)
    VALUES (${appNum}, ${docId || null}, ${date || null}, ${text || null})
    ON CONFLICT (application_number) DO UPDATE SET
      nirc_doc_id = EXCLUDED.nirc_doc_id, nirc_date = EXCLUDED.nirc_date, nirc_text = EXCLUDED.nirc_text`;
}
// Store the AI comparison. Counts are computed here (in code) from the AI's
// structured lists, so the page's percentage is deterministic and re-derivable.
export async function setNircArt(appNum, o) {
  await ensureSchema();
  const reqRefs = Array.isArray(o.req_refs) ? o.req_refs : [];
  const nircRefs = Array.isArray(o.nirc_refs) ? o.nirc_refs : [];
  const matches = Array.isArray(o.matches) ? o.matches : [];
  const inv = matches.filter((m) => m && m.role === 'invalidating').length;
  const dist = matches.filter((m) => m && m.role === 'distinguished').length;
  await sql`
    INSERT INTO reexam_nirc_art
      (application_number, req_refs, nirc_refs, matches, req_count, nirc_count,
       match_count, match_invalidating, match_distinguished, basis, ai_confidence, ai_note, nirc_ai_v, art_source)
    VALUES (${appNum}, ${JSON.stringify(reqRefs)}, ${JSON.stringify(nircRefs)}, ${JSON.stringify(matches)},
            ${reqRefs.length}, ${nircRefs.length}, ${matches.length}, ${inv}, ${dist},
            ${o.basis || null}, ${o.confidence || null}, ${o.note || null}, ${NIRC_AI_V}, 'nirc')
    ON CONFLICT (application_number) DO UPDATE SET
      req_refs = EXCLUDED.req_refs, nirc_refs = EXCLUDED.nirc_refs, matches = EXCLUDED.matches,
      req_count = EXCLUDED.req_count, nirc_count = EXCLUDED.nirc_count, match_count = EXCLUDED.match_count,
      match_invalidating = EXCLUDED.match_invalidating, match_distinguished = EXCLUDED.match_distinguished,
      basis = EXCLUDED.basis, ai_confidence = EXCLUDED.ai_confidence, ai_note = EXCLUDED.ai_note,
      nirc_ai_v = EXCLUDED.nirc_ai_v, art_source = 'nirc',
      -- a changed NIRC verdict clears any prior final-OA fill so it re-processes
      oa_ai_v = NULL, updated_at = now()`;
}

// ── Final-Office-action bucket (second AI pass) ────────────────────
const OA_AI_V = 1;
// NIRC rows whose statement of reasons named no art (not-stated / no-nirc-art)
// but which have request art to match against — candidates for reading the last
// substantial Office action instead. Returns stored req_refs so the OA pass can
// match against them; oa_text is the cached action text if already fetched.
export async function getNircFinalOaToProcess(limit) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT application_number, req_refs, oa_text
    FROM reexam_nirc_art
    WHERE nirc_ai_v IS NOT NULL AND basis IN ('not-stated', 'no-nirc-art')
      AND req_count > 0 AND coalesce(oa_ai_v, 0) < ${OA_AI_V}
    ORDER BY application_number DESC
    LIMIT ${limit}`;
  return rows;
}
export async function countNircFinalOaToProcess() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT count(*)::int AS n FROM reexam_nirc_art
    WHERE nirc_ai_v IS NOT NULL AND basis IN ('not-stated', 'no-nirc-art')
      AND req_count > 0 AND coalesce(oa_ai_v, 0) < ${OA_AI_V}`;
  return rows[0] ? rows[0].n : 0;
}
export async function cacheOaText(appNum, docId, date, code, text) {
  await ensureSchema();
  await sql`UPDATE reexam_nirc_art SET oa_doc_id = ${docId || null}, oa_date = ${date || null},
              oa_code = ${code || null}, oa_text = ${text || null}
            WHERE application_number = ${appNum}`;
}
// Fill a not-stated/no-nirc-art row from its last Office action: the operative
// art (oa_refs) replaces nirc_refs, matches are request refs operative in the
// action, art_source flips to 'final-oa', basis to 'final-oa'.
export async function setNircFinalOa(appNum, o) {
  await ensureSchema();
  const oaRefs = Array.isArray(o.oa_refs) ? o.oa_refs : [];
  const matches = Array.isArray(o.matches) ? o.matches : [];
  const inv = matches.filter((m) => m && m.role === 'invalidating').length;
  const dist = matches.filter((m) => m && m.role === 'distinguished').length;
  // Only flip to the 'final-oa' basis (which counts toward the operative
  // statistic) when the action actually named art. When the staged action was a
  // cover-form-only stub with no identifiable art, keep the row's existing
  // not-stated/no-nirc-art basis so it stays excluded — but still stamp oa_ai_v
  // so it isn't re-processed every night.
  const found = oaRefs.length > 0;
  await sql`UPDATE reexam_nirc_art SET
      nirc_refs = ${JSON.stringify(oaRefs)}, matches = ${JSON.stringify(matches)},
      nirc_count = ${oaRefs.length}, match_count = ${matches.length},
      match_invalidating = ${inv}, match_distinguished = ${dist},
      art_source = ${found ? 'final-oa' : 'nirc'},
      basis = CASE WHEN ${found} THEN 'final-oa' ELSE basis END,
      ai_confidence = ${o.confidence || null}, ai_note = ${o.note || null},
      oa_ai_v = ${OA_AI_V}, updated_at = now()
    WHERE application_number = ${appNum}`;
}
// Persist the operative Office action's identity (doc id / date / code) for a
// final-oa row, without touching oa_text. oa-fetch only cached these when it
// extracted a text layer; image-only actions (OCR'd separately) left them NULL,
// so oa-upload now calls this from the manifest, and oameta.mjs backfills them.
export async function setNircOaMeta(appNum, docId, date, code) {
  await ensureSchema();
  await sql`UPDATE reexam_nirc_art SET oa_doc_id = ${docId || null}, oa_date = ${date || null}, oa_code = ${code || null}
            WHERE application_number = ${appNum}`;
}
export async function getNircFinalOaNeedingMeta(limit) {
  await ensureSchema();
  const { rows } = await sql`SELECT application_number FROM reexam_nirc_art
    WHERE art_source = 'final-oa' AND oa_doc_id IS NULL
    ORDER BY application_number DESC LIMIT ${limit}`;
  return rows;
}
export async function countNircFinalOaNeedingMeta() {
  await ensureSchema();
  const { rows } = await sql`SELECT count(*)::int AS n FROM reexam_nirc_art
    WHERE art_source = 'final-oa' AND oa_doc_id IS NULL`;
  return rows[0] ? rows[0].n : 0;
}
// NIRC rows whose Office-action count hasn't been computed yet (deterministic
// doc-code count, filled by oacount.mjs). Returns the NIRC date so the counter
// only counts actions issued on/before it.
export async function getNircNeedingOaCount(limit) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT n.application_number, c.nirc_date
    FROM reexam_nirc_art n JOIN reexam_conclusions c ON c.application_number = n.application_number
    WHERE n.nirc_ai_v IS NOT NULL AND n.oa_count IS NULL
    ORDER BY n.application_number DESC
    LIMIT ${limit}`;
  return rows;
}
export async function countNircNeedingOaCount() {
  await ensureSchema();
  const { rows } = await sql`SELECT count(*)::int AS n FROM reexam_nirc_art
    WHERE nirc_ai_v IS NOT NULL AND oa_count IS NULL`;
  return rows[0] ? rows[0].n : 0;
}
export async function setNircOaCount(appNum, count) {
  await ensureSchema();
  await sql`UPDATE reexam_nirc_art SET oa_count = ${count} WHERE application_number = ${appNum}`;
}
// Rows for the /reexam-nirc page: the AI comparison joined with the proceeding's
// outcome and NIRC link. Only analyzed rows (nirc_ai_v set) appear.
export async function listNircArt() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT n.application_number,
           c.nirc_doc_id, c.nirc_date,
           n.req_refs, n.nirc_refs, n.matches,
           n.req_count, n.nirc_count, n.match_count, n.match_invalidating, n.match_distinguished,
           n.basis, n.ai_confidence, n.ai_note,
           coalesce(n.art_source, 'nirc') AS art_source, n.oa_doc_id, n.oa_date, n.oa_code, n.oa_count,
           c.outcome_summary, c.cert_doc_id,
           d.group_art_unit, d.examiner_name, d.order_date
    FROM reexam_nirc_art n
    JOIN reexam_conclusions c ON c.application_number = n.application_number
    LEFT JOIN (
      SELECT DISTINCT ON (application_number) application_number, group_art_unit, examiner_name, official_date AS order_date
      FROM reexam_determinations WHERE determination_code = 'RXREXO'
      ORDER BY application_number, official_date ASC NULLS LAST
    ) d ON d.application_number = n.application_number
    WHERE n.nirc_ai_v IS NOT NULL
    ORDER BY n.nirc_date DESC NULLS LAST, n.application_number DESC`;
  return rows;
}
...
// Denied reexams — never ordered, so there is no certificate and therefore no
// "concluded" signal to drop a proceeding from rotation the way ordered ones do.
// A denied reexam can still draw a Petition for Review of the denial (RXRPET)
// months later, so every denied proceeding stays in the pool indefinitely.
// Small population (order of 100), so an indefinite pool is cheap. Reuses the
// same reexam_petition_scan cursor as the ordered-petition check above — safe,
// since a proceeding can't be both ordered and denied, so the two pools never
// overlap on the same application_number.
export async function getDeniedReexamsToCheckPetitions(limit) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT application_number, official_date AS order_date FROM (
      SELECT DISTINCT ON (d.application_number) d.application_number, d.official_date, s.checked_at
      FROM reexam_determinations d
      LEFT JOIN reexam_petition_scan s ON s.application_number = d.application_number
      WHERE d.determination_code = 'RXREXD'
        AND (s.checked_at IS NULL OR s.checked_at < now() - interval '2 days')
      ORDER BY d.application_number, d.official_date ASC NULLS LAST
    ) t
    ORDER BY t.checked_at ASC NULLS FIRST
...
// Record a document code the petition harvest saw but could NOT classify, when
// the code or its description looks petition/request-related. Called only from
// the harvest, which already holds the wrapper -- so this costs no API calls.
// Aggregate (one row per code) so the table stays a short review list rather
// than a log. Surfaced on /status via getStatusSnapshot.
export async function recordUnclassifiedPetitionCode(code, description, appNum, docId) {
  await ensureSchema();
  await sql`
    INSERT INTO petition_code_audit (doc_code, description, seen_count, sample_app, sample_doc_id)
    VALUES (${code}, ${description || null}, 1, ${appNum}, ${docId})
    ON CONFLICT (doc_code) DO UPDATE SET
      seen_count = petition_code_audit.seen_count + 1,
      description = COALESCE(EXCLUDED.description, petition_code_audit.description),
      last_seen = now()`;
}

// ── Law-firm attribution (lib/firms.js reads both sides off the determination) ──
// Bump to re-extract every proceeding after changing the extractor.
// v2: strip office / client qualifiers -- "(DC)", "(SV)", "- Reston", "/ Atmel" --
// which were splitting one firm into a row per office on the scorecard.
// v3: strip the "d/b/a" trade-name tail, written five different ways, which alone
// split Flener IP Law across five keys.
export const FIRM_V = 3;

export async function setReexamFirms(appNum, o) {
  await ensureSchema();
  await sql`
    INSERT INTO reexam_firm (application_number, determination_date, determination_kind,
      owner_firm_raw, owner_firm, owner_firm_key, owner_filer_type,
      requester_firm_raw, requester_firm, requester_firm_key, requester_filer_type,
      firm_v, extracted_at)
    VALUES (${appNum}, ${o.determinationDate || null}, ${o.determinationKind || null},
      ${o.ownerRaw || null}, ${o.ownerFirm || null}, ${o.ownerKey || null}, ${o.ownerType || null},
      ${o.requesterRaw || null}, ${o.requesterFirm || null}, ${o.requesterKey || null}, ${o.requesterType || null},
      ${FIRM_V}, now())
    ON CONFLICT (application_number) DO UPDATE SET
      determination_date = EXCLUDED.determination_date,
      determination_kind = EXCLUDED.determination_kind,
      owner_firm_raw = EXCLUDED.owner_firm_raw, owner_firm = EXCLUDED.owner_firm,
      owner_firm_key = EXCLUDED.owner_firm_key, owner_filer_type = EXCLUDED.owner_filer_type,
      requester_firm_raw = EXCLUDED.requester_firm_raw, requester_firm = EXCLUDED.requester_firm,
      requester_firm_key = EXCLUDED.requester_firm_key, requester_filer_type = EXCLUDED.requester_filer_type,
      firm_v = EXCLUDED.firm_v, extracted_at = now()`;
}

// Every attributed proceeding, both sides, for the firm-statistics page. Raw rows
// (not aggregates) so the page can re-group as its thresholds change and drill
// into a firm's proceedings without another round trip. Joined to the patent and
// requester party name so a firm's row list is readable.
export async function listReexamFirms() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT f.application_number, f.determination_date, f.determination_kind,
           f.owner_firm, f.owner_firm_key, f.owner_filer_type,
           f.requester_firm, f.requester_firm_key, f.requester_filer_type,
           t.underlying_patent, t.patent_owner, rq.requester_name
      FROM reexam_firm f
      LEFT JOIN reexam_tech_center t ON t.application_number = f.application_number
      LEFT JOIN reexam_requester rq ON rq.application_number = f.application_number
     ORDER BY f.determination_date DESC NULLS LAST, f.application_number DESC`;
  return rows;
}

// Drop audit rows for codes the classifier now recognizes. Without this a code
// stays on the review list forever once added -- it stops incrementing, but the
// stale row lingers as noise. Codes are passed in (rather than imported) because
// the classifier lives in lib/petitions.js, which imports from here; the sweep,
// which sees every code in the corpus, is the natural caller.
export async function pruneClassifiedPetitionCodes(codes) {
  await ensureSchema();
  const list = [...new Set((codes || []).filter(Boolean))];
  if (!list.length) return 0;
  const { rowCount } = await sql`DELETE FROM petition_code_audit WHERE doc_code = ANY(${list})`;
  return rowCount;
}

// The review list: unrecognized petition-ish codes, most frequent first.
async function listUnclassifiedPetitionCodes() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT doc_code, description, seen_count, sample_app, sample_doc_id, first_seen, last_seen
      FROM petition_code_audit ORDER BY seen_count DESC, doc_code`;
  return rows;
}

// Cross-reference audit: post-order patent-owner petitions (the narrower page)
// whose petition document has no counterpart in the broader petition-trail
// table. The broader page is meant to be a superset, so a non-zero count here
// means either a classifier gap (a code we drop) or a stale cached doc_id (the
// document no longer resolves in USPTO's live feed under any code). This is the
// check that surfaced RXRQ/T.
async function getPetitionSupersetGaps() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT p.application_number, p.petition_doc_id, p.petition_date,
           EXISTS (SELECT 1 FROM reexam_petition_docs d
                    WHERE d.application_number = p.application_number AND d.kind = 'petition') AS has_any_petition
      FROM reexam_post_petitions p
      LEFT JOIN reexam_petition_docs d ON d.doc_id = p.petition_doc_id
     WHERE p.petition_doc_id IS NOT NULL AND d.doc_id IS NULL
     ORDER BY p.application_number`;
  return rows;
}
...
export async function recordPostPetition(appNum, p) {
  ...
  await sql`
    ...
    ON CONFLICT (application_number) DO UPDATE SET
      ...
      -- When ANY heuristic pick changes (new petition/opposition/decision posted),
      -- clear the AI-verification gate so the nightly pass re-reads the documents.
      post_ai_v = CASE WHEN reexam_post_petitions.petition_doc_id   IS DISTINCT FROM EXCLUDED.petition_doc_id
                         OR reexam_post_petitions.opposition_doc_id IS DISTINCT FROM EXCLUDED.opposition_doc_id
                         OR reexam_post_petitions.decision_doc_id   IS DISTINCT FROM EXCLUDED.decision_doc_id
                       THEN NULL ELSE reexam_post_petitions.post_ai_v END`;
...
// AI-attribution overlay: once verified (post_ai_v set), the AI's document-read
// picks replace the timing-heuristic ones — a row whose "petition" turns out
// not to be a PO § 325(d) petition drops off the page, and the review flag is
// cleared (the role was confirmed from the documents, not a page-count tie).
export async function listPostOrderPetitions() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT p.application_number, p.order_date,
           CASE WHEN p.post_ai_v IS NOT NULL THEN p.ai_pet_doc_id ELSE p.petition_doc_id END AS petition_doc_id,
           CASE WHEN p.post_ai_v IS NOT NULL THEN p.ai_pet_date   ELSE p.petition_date   END AS petition_date,
           CASE WHEN p.post_ai_v IS NOT NULL THEN p.ai_opp_doc_id ELSE p.opposition_doc_id END AS opposition_doc_id,
           CASE WHEN p.post_ai_v IS NOT NULL THEN p.ai_opp_date   ELSE p.opposition_date   END AS opposition_date,
           CASE WHEN p.post_ai_v IS NOT NULL THEN p.ai_dec_doc_id ELSE p.decision_doc_id END AS decision_doc_id,
           CASE WHEN p.post_ai_v IS NOT NULL THEN p.ai_dec_date   ELSE p.decision_date   END AS decision_date,
           CASE WHEN p.post_ai_v IS NOT NULL THEN p.ai_dec_outcome ELSE p.decision_outcome END AS decision_outcome,
           CASE WHEN p.post_ai_v IS NOT NULL THEN p.ai_dec_325d   ELSE p.decision_325d   END AS decision_325d,
           p.decision_pdf_blob_url,
           CASE WHEN p.post_ai_v IS NOT NULL THEN false ELSE p.review_flag END AS review_flag,
    ...
    WHERE p.order_date >= '2025-01-01'
      ...
      -- Once AI-verified, the petition must exist AND cite 325(d). ai_pet_325d
      -- can be true with a NULL doc id: the petition was filed but its document
      -- is not publicly available (e.g. expunged) — still a real 325(d) row.
      AND CASE WHEN p.post_ai_v IS NOT NULL
               THEN (p.ai_pet_doc_id IS NOT NULL OR p.ai_pet_325d = true) AND p.ai_pet_325d IS DISTINCT FROM false
               ELSE p.petition_325d IS DISTINCT FROM false END
      ...
      AND CASE WHEN p.post_ai_v IS NOT NULL
               THEN p.ai_dec_325d IS DISTINCT FROM false
               ELSE p.decision_325d IS DISTINCT FROM false END
    ORDER BY CASE WHEN p.post_ai_v IS NOT NULL THEN p.ai_pet_date ELSE p.petition_date END DESC NULLS LAST,
...
// A determination's code + official date, keyed by the (possibly stale) document
// identifier we stored. Used by /api/document to recover a re-issued determination
// PDF by matching the current documents feed on its code. No ensureSchema — this
// is a hot-path read against an existing table.
export async function getDeterminationByDoc(appNum, docId) {
  const { rows } = await sql`
    SELECT determination_code AS code, determination_type AS type, official_date
    FROM reexam_determinations
    WHERE application_number = ${String(appNum)} AND document_identifier = ${String(docId)}
    LIMIT 1`;
  return rows[0] || null;
}
...
// Record what the last digest attempt actually did, so a failure shows on /status
// instead of living only in an HTTP response nobody reads. Written on every
// attempt — success, empty day, or failure.
export async function setSubDigestResult(result) {
  await ensureSchema();
  await sql`UPDATE reexam_state SET last_sub_digest_result = ${JSON.stringify(result || {})}::jsonb WHERE id = 1`;
}
  ...
  const { rows } = await sql`
    ...
    WHERE LEFT(e.official_date, 10) = ${String(date)}
      AND (e.category NOT LIKE 'post_%'
           OR CASE WHEN p.post_ai_v IS NOT NULL
                   THEN (p.ai_pet_doc_id IS NOT NULL OR p.ai_pet_325d = true)
                        AND p.ai_pet_325d IS DISTINCT FROM false AND p.ai_dec_325d IS DISTINCT FROM false
                   ELSE p.petition_325d IS DISTINCT FROM false AND p.decision_325d IS DISTINCT FROM false END)
    ORDER BY e.category, e.application_number`;
  return rows;
}

// Patent owner + third-party requester for a set of control numbers, so digest
// rows can name the parties instead of showing a bare control number. Same three
// sources the /reexam table uses: owner from reexam_tech_center (ODP applicant of
// record), the authoritative requester NAME from the request document, and the
// requester TYPE from the latest determination (which distinguishes a patent-owner
// -requested reexam from a third-party one).
export async function getReexamPartiesByApps(appNums) {
  await ensureSchema();
  const list = [...new Set((appNums || []).filter(Boolean).map(String))];
  if (!list.length) return new Map();
  const { rows } = await sql`
    SELECT a.application_number, tc.patent_owner,
           rq.requester_name, rq.ai_confidence AS requester_confidence,
           d.requester_type
    FROM (SELECT DISTINCT application_number FROM reexam_doc_events
          WHERE application_number = ANY(${list})) a
    LEFT JOIN reexam_tech_center tc ON tc.application_number = a.application_number
    LEFT JOIN reexam_requester rq   ON rq.application_number = a.application_number
    LEFT JOIN (SELECT DISTINCT ON (application_number) application_number, requester_type
               FROM reexam_determinations
               ORDER BY application_number, official_date DESC NULLS LAST) d
      ON d.application_number = a.application_number`;
  const out = new Map();
  for (const r of rows) out.set(r.application_number, r);
  return out;
}
...
export async function getStatusSnapshot() {
  ...
  const { rows } = await sql`
    SELECT
      ...
      (SELECT last_sub_digest_result FROM reexam_state WHERE id = 1) AS subscriber_digest_result,
      ...
      (SELECT v FROM ptab_kv WHERE k = 'pdfcount_filehistory') AS pdf_filehistory_count,
      (SELECT v FROM ptab_kv WHERE k = 'pdfcount_docmerge') AS pdf_docmerge_count,
      -- Coverage audits: unrecognized petition-ish document codes seen during
      -- harvest, and post-order petitions with no counterpart in the broader
      -- petition-trail table. Both should normally be 0; non-zero = review.
      (SELECT count(*)::int FROM petition_code_audit) AS unclassified_codes,
      (SELECT count(*)::int FROM reexam_post_petitions p
         LEFT JOIN reexam_petition_docs d ON d.doc_id = p.petition_doc_id
         WHERE p.petition_doc_id IS NOT NULL AND d.doc_id IS NULL) AS petition_superset_gaps,
      (SELECT max(official_date) FROM reexam_determinations) AS latest_determination`;
  const s = rows[0] || {};
  s.pdf_filehistory_count = parseInt(s.pdf_filehistory_count, 10) || 0;
  s.pdf_docmerge_count = parseInt(s.pdf_docmerge_count, 10) || 0;
  // 7-day daily breakdown for the PDF export counters. Buckets are written by the
  // ?bump= handler as pdfcount_<name>_<YYYY-MM-DD> in PT (oldest→newest, ending today).
  const days = [];
  for (let i = 6; i >= 0; i--) {
    days.push(new Date(Date.now() - i * 86400000).toLocaleDateString('en-CA', { timeZone: 'America/Los_Angeles' }));
  }
  const dayKeys = days.flatMap((d) => [`pdfcount_filehistory_${d}`, `pdfcount_docmerge_${d}`]);
  const daily = await getCounters(dayKeys);
  s.pdf_filehistory_daily = days.map((d) => ({ d, count: daily[`pdfcount_filehistory_${d}`] || 0 }));
  s.pdf_docmerge_daily = days.map((d) => ({ d, count: daily[`pdfcount_docmerge_${d}`] || 0 }));
  // Coverage-audit detail, so /status shows WHICH codes and proceedings need a
  // look rather than only a count. Both lists are short by construction.
  s._unclassifiedCodes = await listUnclassifiedPetitionCodes();
  s._supersetGaps = await getPetitionSupersetGaps();
  return s;
}

// ── Reexam related-litigation pass (RXLITSR search report, nightly local Claude) ──
// Bump to re-extract every reexam's litigation from its RXLITSR document.
const RXLIT_V = 1;
// Reexams whose litigation hasn't been extracted at the current version. Recent
// first (control-number DESC) so a partial slice covers the newest proceedings.
export async function getReexamsNeedingLitigation(limit) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT DISTINCT d.application_number, lit.litsr_doc_id, lit.litsr_text
    FROM reexam_determinations d
    LEFT JOIN reexam_litigation lit ON lit.application_number = d.application_number
    WHERE coalesce(lit.rxlit_v, 0) < ${RXLIT_V}
    ORDER BY d.application_number DESC
    LIMIT ${limit}`;
  return rows;
}
export async function countReexamsNeedingLitigation() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT count(*)::int AS n FROM (
...
// Court-context snippets from the determination text — the ~90 chars around each
// district-court docket number — so the extractor can attach a spelled-out court
// (e.g. "(E.D. Tex.)") to an RXLITSR case that gave only a docket number.
export async function getReexamCourtHints(appNum) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT string_agg(DISTINCT m[1], ' | ') AS hints
    FROM reexam_doc_text t,
         LATERAL regexp_matches(t.text, '.{0,90}[0-9]:[0-9]{2}-?cv-?[0-9]{2,6}.{0,70}', 'g') AS m
    WHERE t.application_number = ${appNum} AND t.doc_kind IN ('order', 'denial')`;
  return rows[0] && rows[0].hints ? rows[0].hints.replace(/\s+/g, ' ').slice(0, 4000) : '';
}
// Stage the RXLITSR doc identity + OCR text (fetch step); rxlit_v stays 0 until
// the extraction uploads, so the row is still "needing" until then.
export async function cacheRxLitsrText(appNum, docId, date, text) {
  await ensureSchema();
  await sql`INSERT INTO reexam_litigation (application_number, litsr_doc_id, litsr_date, litsr_text)
    VALUES (${appNum}, ${docId || null}, ${date || null}, ${text || null})
    ON CONFLICT (application_number) DO UPDATE SET
      litsr_doc_id = EXCLUDED.litsr_doc_id, litsr_date = EXCLUDED.litsr_date,
      litsr_text = EXCLUDED.litsr_text, updated_at = now()`;
}
// Mark a reexam that has no RXLITSR document so it isn't reprocessed nightly.
export async function markReexamNoLitsr(appNum) {
  await ensureSchema();
  await sql`INSERT INTO reexam_litigation (application_number, cases, none_found, ai_note, rxlit_v)
    VALUES (${appNum}, '[]'::jsonb, false, 'no RXLITSR document', ${RXLIT_V})
    ON CONFLICT (application_number) DO UPDATE SET
      ai_note = 'no RXLITSR document', rxlit_v = ${RXLIT_V}, updated_at = now()`;
}
// Store the extracted litigation (upload step). cases: [{caption, case_no, court, status}].
export async function setReexamLitigation(appNum, o) {
  await ensureSchema();
  const cases = Array.isArray(o.cases) ? o.cases : [];
  await sql`INSERT INTO reexam_litigation (application_number, cases, none_found, ai_confidence, ai_note, rxlit_v)
    VALUES (${appNum}, ${JSON.stringify(cases)}, ${!!o.none_found}, ${o.confidence || null}, ${o.note || null}, ${RXLIT_V})
    ON CONFLICT (application_number) DO UPDATE SET
      cases = ${JSON.stringify(cases)}, none_found = ${!!o.none_found},
      ai_confidence = ${o.confidence || null}, ai_note = ${o.note || null},
      rxlit_v = ${RXLIT_V}, updated_at = now()`;
}

// ── Third-party requester identity (from the reexam REQUEST document) ──
const REQID_V = 1;
// Third-party reexams whose requester identity hasn't been extracted at the
// current version. (Patent-owner-requested reexams need no lookup — the requester
// is the patent owner.) Recent first.
export async function getReexamsNeedingRequester(limit) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT DISTINCT d.application_number, rq.req_doc_id, rq.req_text
    FROM reexam_determinations d
    LEFT JOIN reexam_requester rq ON rq.application_number = d.application_number
    WHERE d.requester_type = 'third_party' AND coalesce(rq.reqid_v, 0) < ${REQID_V}
    ORDER BY d.application_number DESC
    LIMIT ${limit}`;
  return rows;
}
export async function countReexamsNeedingRequester() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT count(*)::int AS n FROM (
      SELECT DISTINCT d.application_number FROM reexam_determinations d
...
// Stage the request doc identity + OCR text (fetch step); reqid_v stays 0 until upload.
export async function cacheRequesterText(appNum, docId, date, code, text) {
  await ensureSchema();
  await sql`INSERT INTO reexam_requester (application_number, req_doc_id, req_date, req_code, req_text)
    VALUES (${appNum}, ${docId || null}, ${date || null}, ${code || null}, ${text || null})
    ON CONFLICT (application_number) DO UPDATE SET
      req_doc_id = EXCLUDED.req_doc_id, req_date = EXCLUDED.req_date,
      req_code = EXCLUDED.req_code, req_text = EXCLUDED.req_text, updated_at = now()`;
}
// Mark a third-party reexam that has no request document on file (don't reprocess).
export async function markNoRequestDoc(appNum) {
  await ensureSchema();
  await sql`INSERT INTO reexam_requester (application_number, ai_note, reqid_v)
    VALUES (${appNum}, 'no request document', ${REQID_V})
    ON CONFLICT (application_number) DO UPDATE SET
      ai_note = 'no request document', reqid_v = ${REQID_V}, updated_at = now()`;
}
// Store the extracted requester identity (upload step).
export async function setReexamRequester(appNum, o) {
  await ensureSchema();
  await sql`INSERT INTO reexam_requester (application_number, requester_name, ai_confidence, ai_note, reqid_v)
    VALUES (${appNum}, ${o.requester_name || null}, ${o.confidence || null}, ${o.note || null}, ${REQID_V})
    ON CONFLICT (application_number) DO UPDATE SET
      requester_name = ${o.requester_name || null}, ai_confidence = ${o.confidence || null},
      ai_note = ${o.note || null}, reqid_v = ${REQID_V}, updated_at = now()`;
}

// ── TRNA fallback: recover requester identity from the Transmittal of New
// Application (doc code TRNA), which carries a structured field "The name and
// address of the person requesting reexamination is:". Used only to FILL gaps
// the RXOSUB.R request-document pass left empty — it never overwrites a name.
// Third-party reexams that still have no requester name AND have not yet been
// checked via TRNA (req_code = 'TRNA' is stamped by setRequesterFromTrna on every
// row it touches, including counsel-only fields that yield no party, so a TRNA
// pass is attempted at most once per proceeding). Recent first.
export async function getReexamsMissingRequester(limit) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT d.application_number, rq.req_code AS prior_code
    FROM reexam_determinations d
    JOIN reexam_requester rq ON rq.application_number = d.application_number
    WHERE d.requester_type = 'third_party'
      AND (rq.requester_name IS NULL OR rq.requester_name = '')
      AND rq.req_code IS DISTINCT FROM 'TRNA'
    GROUP BY d.application_number, rq.req_code
    ORDER BY d.application_number DESC
    LIMIT ${limit}`;
  return rows;
}
// Fill requester_name from the TRNA field, recording TRNA provenance. Guarded so
// it only writes when the row still has no name — a good name is never clobbered.
export async function setRequesterFromTrna(appNum, o) {
  await ensureSchema();
  await sql`INSERT INTO reexam_requester (application_number, requester_name, req_doc_id, req_date, req_code, ai_confidence, ai_note, reqid_v)
    VALUES (${appNum}, ${o.requester_name || null}, ${o.docId || null}, ${o.date || null}, 'TRNA', ${o.confidence || null}, ${o.note || null}, ${REQID_V})
    ON CONFLICT (application_number) DO UPDATE SET
      requester_name = ${o.requester_name || null}, req_doc_id = ${o.docId || null},
      req_date = ${o.date || null}, req_code = 'TRNA', ai_confidence = ${o.confidence || null},
      ai_note = ${o.note || null}, updated_at = now()
      WHERE reexam_requester.requester_name IS NULL OR reexam_requester.requester_name = ''`;
}

// ── Petition-trail documents (all petitions/oppositions/decisions per reexam) ──
// Idempotent bulk upsert of the petition-related docs seen in one proceeding's
// documents feed (classified by classifyPetitionDoc in lib/petitions.js).
export async function recordPetitionDocs(appNum, docs) {
  await ensureSchema();
  for (const d of docs) {
    if (!d.doc_id) continue;
    await sql`INSERT INTO reexam_petition_docs (doc_id, application_number, official_date, doc_code, kind, outcome)
      VALUES (${d.doc_id}, ${appNum}, ${d.official_date || null}, ${d.doc_code || null}, ${d.kind}, ${d.outcome || null})
      ON CONFLICT (doc_id) DO UPDATE SET
        official_date = EXCLUDED.official_date, doc_code = EXCLUDED.doc_code,
        kind = EXCLUDED.kind, outcome = EXCLUDED.outcome`;
  }
}
// ── Petition subject matter (OCR + AI over each decision; PETSUBJ_V gates re-runs) ──
const PETSUBJ_V = 1;
// Decision documents whose subject matter hasn't been classified at this version.
export async function getDecisionsNeedingSubject(limit) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT p.doc_id, p.application_number, p.official_date, p.doc_code, p.outcome
    FROM reexam_petition_docs p
    LEFT JOIN reexam_petition_subject s ON s.doc_id = p.doc_id
    WHERE p.kind = 'decision' AND coalesce(s.petsubj_v, 0) < ${PETSUBJ_V}
    ORDER BY p.official_date DESC NULLS LAST
    LIMIT ${limit}`;
  return rows;
}
export async function countDecisionsNeedingSubject() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT count(*)::int AS n FROM reexam_petition_docs p
    LEFT JOIN reexam_petition_subject s ON s.doc_id = p.doc_id
    WHERE p.kind = 'decision' AND coalesce(s.petsubj_v, 0) < ${PETSUBJ_V}`;
  return rows[0] ? rows[0].n : 0;
}
export async function setPetitionSubject(docId, appNum, o) {
  await ensureSchema();
  await sql`INSERT INTO reexam_petition_subject
      (doc_id, application_number, reliefs, primary_relief, merits_outcome, ancillary_waiver,
       granted_relief, referred_to_cru,
       rules, statutes, petitioner, relief_verbatim, ai_confidence, ai_note, petsubj_v)
    VALUES (${docId}, ${appNum}, ${o.reliefs || null}, ${o.primary_relief || null},
       ${o.merits_outcome || null}, ${o.ancillary_waiver || null},
       ${o.granted_relief || null}, ${o.referred_to_cru === true},
       ${o.rules || null}, ${o.statutes || null}, ${o.petitioner || null},
       ${o.relief_verbatim || null}, ${o.confidence || null}, ${o.note || null}, ${PETSUBJ_V})
    ON CONFLICT (doc_id) DO UPDATE SET
      application_number = EXCLUDED.application_number, reliefs = EXCLUDED.reliefs,
      primary_relief = EXCLUDED.primary_relief, merits_outcome = EXCLUDED.merits_outcome,
      ancillary_waiver = EXCLUDED.ancillary_waiver, granted_relief = EXCLUDED.granted_relief,
      referred_to_cru = EXCLUDED.referred_to_cru, rules = EXCLUDED.rules,
      statutes = EXCLUDED.statutes, petitioner = EXCLUDED.petitioner,
      relief_verbatim = EXCLUDED.relief_verbatim, ai_confidence = EXCLUDED.ai_confidence,
      ai_note = EXCLUDED.ai_note, petsubj_v = ${PETSUBJ_V}, updated_at = now()`;
}

// ── Relief as filed, from the petition document (PETREQ_V gates re-runs) ──
const PETREQ_V = 1;
// Petition documents whose as-filed relief hasn't been extracted at this version.
// Ordered so petitions in proceedings with NO decision come first — those are the
// rows that currently show no relief at all on the page, so they carry the most
// user-visible value if a run is interrupted.
export async function getPetitionsNeedingRequestSubject(limit) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT p.doc_id, p.application_number, p.official_date, p.doc_code,
           (NOT EXISTS (SELECT 1 FROM reexam_petition_docs d
                        WHERE d.application_number = p.application_number AND d.kind = 'decision')) AS undecided
    FROM reexam_petition_docs p
    LEFT JOIN reexam_petition_request r ON r.doc_id = p.doc_id
    WHERE p.kind = 'petition' AND coalesce(r.petreq_v, 0) < ${PETREQ_V}
    ORDER BY undecided DESC, p.official_date DESC NULLS LAST
    LIMIT ${limit}`;
  return rows;
}
export async function countPetitionsNeedingRequestSubject() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT count(*)::int AS n FROM reexam_petition_docs p
    LEFT JOIN reexam_petition_request r ON r.doc_id = p.doc_id
...
export async function setPetitionRequestSubject(docId, appNum, o) {
  await ensureSchema();
  const isPet = o.is_petition === false ? false : true;
  await sql`INSERT INTO reexam_petition_request
      (doc_id, application_number, reliefs, primary_relief, rules, statutes,
       petitioner, relief_verbatim, ai_confidence, ai_note, is_petition, petreq_v)
    VALUES (${docId}, ${appNum}, ${o.reliefs || null}, ${o.primary_relief || null},
       ${o.rules || null}, ${o.statutes || null}, ${o.petitioner || null},
       ${o.relief_verbatim || null}, ${o.confidence || null}, ${o.note || null}, ${isPet}, ${PETREQ_V})
    ON CONFLICT (doc_id) DO UPDATE SET
      application_number = EXCLUDED.application_number, reliefs = EXCLUDED.reliefs,
      primary_relief = EXCLUDED.primary_relief, rules = EXCLUDED.rules,
      statutes = EXCLUDED.statutes, petitioner = EXCLUDED.petitioner,
      relief_verbatim = EXCLUDED.relief_verbatim, ai_confidence = EXCLUDED.ai_confidence,
      ai_note = EXCLUDED.ai_note, is_petition = EXCLUDED.is_petition,
      petreq_v = ${PETREQ_V}, updated_at = now()`;
}

// ── What an opposition answers (PETOPP_V gates re-runs) ──
const PETOPP_V = 1;
// Oppositions not yet read at this version. Ordered so the ones the gap rule
// could not place come first: those are the papers whose target is genuinely in
// question, and the ones where reading the caption changes a row on the page.
export async function getOppositionsNeedingSubject(limit) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT p.doc_id, p.application_number, p.official_date, p.doc_code,
           (SELECT min(q.official_date) FROM reexam_petition_docs q
             WHERE q.application_number = p.application_number AND q.kind = 'petition'
               AND q.official_date <= p.official_date) AS nearest_petition_date
    FROM reexam_petition_docs p
    LEFT JOIN reexam_petition_opposition o ON o.doc_id = p.doc_id
    WHERE p.kind = 'opposition' AND coalesce(o.petopp_v, 0) < ${PETOPP_V}
    ORDER BY p.official_date DESC NULLS LAST
    LIMIT ${limit}`;
  return rows;
}
export async function countOppositionsNeedingSubject() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT count(*)::int AS n FROM reexam_petition_docs p
    LEFT JOIN reexam_petition_opposition o ON o.doc_id = p.doc_id
...
export async function setPetitionOppositionSubject(docId, appNum, o) {
  await ensureSchema();
  const isOpp = o.is_opposition === false ? false : true;
  await sql`INSERT INTO reexam_petition_opposition
      (doc_id, application_number, party, opposes_date, opposes_verbatim,
       is_opposition, ai_confidence, ai_note, petopp_v)
    VALUES (${docId}, ${appNum}, ${o.party || null}, ${o.opposes_date || null},
       ${o.opposes_verbatim || null}, ${isOpp}, ${o.confidence || null},
       ${o.note || null}, ${PETOPP_V})
    ON CONFLICT (doc_id) DO UPDATE SET
      application_number = EXCLUDED.application_number, party = EXCLUDED.party,
      opposes_date = EXCLUDED.opposes_date, opposes_verbatim = EXCLUDED.opposes_verbatim,
      is_opposition = EXCLUDED.is_opposition, ai_confidence = EXCLUDED.ai_confidence,
      ai_note = EXCLUDED.ai_note, petopp_v = ${PETOPP_V}, updated_at = now()`;
}

// The universe of proceedings searched for petitions, so the page can state its
// scope precisely instead of claiming "every" petition. Two entry paths: a
// determination (order/denial) was captured, or the request is recent enough to
// still sit in the rolling watch window awaiting one. Numbers are computed live
// so the disclosure can't drift from the data.
export async function getPetitionUniverse() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT
      (SELECT count(*)::int FROM (
        SELECT application_number FROM reexam_watch
        UNION SELECT application_number FROM reexam_determinations) s) AS tracked,
      (SELECT count(DISTINCT application_number)::int FROM reexam_determinations) AS with_determination,
      (SELECT count(*)::int FROM (
        SELECT w.application_number FROM reexam_watch w
        LEFT JOIN reexam_determinations d ON d.application_number = w.application_number
        WHERE d.application_number IS NULL GROUP BY w.application_number) s) AS awaiting_determination,
      (SELECT min(official_date)::date FROM reexam_determinations) AS earliest_determination,
      (SELECT min(official_date) FROM reexam_petition_docs) AS earliest_petition,
      (SELECT count(DISTINCT application_number)::int FROM reexam_petition_docs) AS proceedings_with_petitions`;
  return rows[0] || {};
}

// All petition-trail docs with proceeding context, for /reexam-petition-decisions.
// Threading into petition→opposition→decision rows happens in the API handler.
export async function listPetitionTrailDocs() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT p.doc_id, p.application_number, p.official_date, p.doc_code, p.kind, p.outcome,
           tc.underlying_patent, tc.patent_owner, rq.requester_name,
           d.requester_type, c.cert_date,
           s.reliefs, s.primary_relief, s.merits_outcome, s.ancillary_waiver,
           s.granted_relief, s.referred_to_cru,
           s.rules, s.petitioner AS pet_party, s.relief_verbatim, s.ai_confidence AS subj_confidence,
           det.determination_type AS det_type, det.det_date,
           r.reliefs AS req_reliefs, r.primary_relief AS req_primary_relief,
           r.relief_verbatim AS req_verbatim, r.petitioner AS req_party,
           r.ai_confidence AS req_confidence,
           coalesce(r.is_petition, true) AS req_is_petition, r.ai_note AS req_note,
           o.party AS opp_party, o.opposes_date AS opp_opposes_date,
           o.opposes_verbatim AS opp_verbatim, o.ai_confidence AS opp_confidence,
           coalesce(o.is_opposition, true) AS opp_is_opposition
    FROM reexam_petition_docs p
    LEFT JOIN reexam_petition_subject s ON s.doc_id = p.doc_id
    LEFT JOIN reexam_petition_request r ON r.doc_id = p.doc_id
    LEFT JOIN reexam_petition_opposition o ON o.doc_id = p.doc_id
    LEFT JOIN reexam_tech_center tc ON tc.application_number = p.application_number
    LEFT JOIN reexam_requester rq ON rq.application_number = p.application_number
    LEFT JOIN (SELECT DISTINCT ON (application_number) application_number, requester_type
               FROM reexam_determinations ORDER BY application_number, official_date DESC) d
      ON d.application_number = p.application_number
    LEFT JOIN reexam_conclusions c ON c.application_number = p.application_number
    -- The proceeding's operative determination. For petitions whose § 325(d) ask
    -- was referred to the CRU, this is where that question was actually answered.
    LEFT JOIN (SELECT DISTINCT ON (application_number) application_number,
                      determination_type, official_date AS det_date
               FROM reexam_determinations
               ORDER BY application_number, official_date DESC NULLS LAST) det
      ON det.application_number = p.application_number
    ORDER BY p.application_number, p.official_date ASC NULLS LAST, p.doc_id`;
  return rows;
}
    ...
    const { rows } = await sql`
      SELECT * FROM (
        SELECT DISTINCT ON (d.application_number)
               ...
               dt.d325_addressed, dt.d325_summary,
               count(*) OVER (PARTITION BY d.application_number)::int AS doc_count
        FROM reexam_determinations d
        LEFT JOIN reexam_watch w ON w.application_number = d.application_number
        LEFT JOIN reexam_conclusions c ON c.application_number = d.application_number
        LEFT JOIN reexam_tech_center tc ON tc.application_number = d.application_number
        LEFT JOIN reexam_doc_text dt ON dt.doc_id = d.document_identifier
  ...
  const { rows } = await sql`
    SELECT * FROM (
      SELECT DISTINCT ON (d.application_number)
             ...
             c.ai_confirmed, c.ai_cancelled, c.ai_amended, c.ai_new, c.ai_summary, c.ai_confidence, c.ai_note, c.cert_ai_v,
             tc.tech_center, tc.underlying_patent, tc.patent_owner,
             -- Only WHETHER a § 325(d) summary exists, not the summary itself. The
             -- text is 636 KB across the corpus -- 27% of this response -- and the
             -- page keeps it inside a collapsed <details>, so a visitor who opens
             -- one row was paying to transfer all 1,542. Fetched per row on demand
             -- via getD325SummariesByDocIds; the limited variant above still
             -- carries the text, because the subscriber digest emails it.
             dt.d325_addressed, (coalesce(dt.d325_summary, '') <> '') AS has_d325_summary,
             lit.cases AS lit_cases, lit.none_found AS lit_none_found,
             lit.litsr_doc_id AS lit_doc_id, lit.litsr_date AS lit_date,
             rq.requester_name, rq.ai_confidence AS requester_confidence,
             count(*) OVER (PARTITION BY d.application_number)::int AS doc_count
      FROM reexam_determinations d
      LEFT JOIN reexam_watch w ON w.application_number = d.application_number
