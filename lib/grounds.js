...
export function extractReferenceNames(text) {
  ...
  const names = new Set();
  const push = (w) => {
    const s = String(w || '').replace(/['’].*$/, '').toLowerCase(); // drop possessive
    if (s.length >= 3 && !NAME_STOP.has(s)) names.add(s);
  };
  // "over X" / "anticipated by X" — require a space (these words are common).
  const strong = /(?:\bover|anticipated\s+by)\s+([A-Z][A-Za-z'’]{2,})/g;
  // "in view of X" / "in combination with X" — allow OCR-joined "ofName".
  const soft = /(?:view\s*of|combination\s*with)\s*([A-Z][A-Za-z'’]{2,})/g;
  let m;
  while ((m = strong.exec(t))) push(m[1]);
  while ((m = soft.exec(t))) push(m[1]);
  return [...names].sort();
}
