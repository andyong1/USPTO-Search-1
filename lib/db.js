...
const SCHEMA_VERSION = '2026-07-18.5';
...
// Distinct reexam underlying-patents due for a proceedings scan: never scanned, or
// last checked before `staleBefore`. Least-recently-checked first (resumable).
export async function getPatentsToScanForProceedings(limit, staleBefore) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT DISTINCT tc.underlying_patent AS patent, s.checked_at
    FROM reexam_tech_center tc
    JOIN reexam_determinations d ON d.application_number = tc.application_number
    LEFT JOIN patent_proceedings_scan s ON s.patent_number = tc.underlying_patent
    WHERE coalesce(tc.underlying_patent, '') <> ''
...
export async function setDocGrounds(docId, refs, trials, cites325d) {
  await sql`UPDATE reexam_doc_text
            SET ref_numbers = ${refs}, trial_mentions = ${trials}, cites_325d = ${!!cites325d}, grounds_v = ${GROUNDS_V}
            WHERE doc_id = ${docId}`;
}
...
export async function getReexamGroundsMap() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT application_number, ref_numbers, trial_mentions, cites_325d FROM reexam_doc_text
    WHERE coalesce(grounds_v, 0) >= ${GROUNDS_V}`;
  const map = new Map();
  for (const r of rows) {
    const e = map.get(r.application_number) || { refs: new Set(), trials: new Set(), cites325d: false };
  ...
  const out = new Map();
  for (const [k, v] of map) out.set(k, { refs: [...v.refs], trials: [...v.trials], cites325d: v.cites325d });
  return out;
}
...
// Trials (from the all-AIA-years linkage table) whose petition references haven't
// been extracted at the current version — for the network-bound ?petrefs=1 step.
export async function getTrialsToExtractPetitionRefs(limit) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT DISTINCT pp.trial_number
    FROM patent_proceedings pp
    LEFT JOIN ptab_petition_refs pr ON pr.trial_number = pp.trial_number
    WHERE coalesce(pp.trial_number, '') <> ''
      AND (pr.trial_number IS NULL OR coalesce(pr.grounds_v, 0) < ${GROUNDS_V})
    LIMIT ${limit}`;
  return rows.map((r) => r.trial_number);
}
// Store a trial's petition references (empty array marks "checked, none found" so
// it isn't retried every run). petDocId null when no petition doc was located.
export async function setPetitionRefs(trialNumber, refs, petDocId) {
  await sql`
    INSERT INTO ptab_petition_refs (trial_number, pet_doc_id, ref_numbers, grounds_v, checked_at)
    VALUES (${trialNumber}, ${petDocId || null}, ${refs}, ${GROUNDS_V}, now())
    ON CONFLICT (trial_number) DO UPDATE SET
      pet_doc_id = EXCLUDED.pet_doc_id, ref_numbers = EXCLUDED.ref_numbers,
      grounds_v = EXCLUDED.grounds_v, checked_at = now()`;
}
export async function countTrialsToExtractPetitionRefs() {
  await ensureSchema();
  const { rows } = await sql`
    SELECT count(*)::int AS n FROM (
      SELECT DISTINCT pp.trial_number FROM patent_proceedings pp
      LEFT JOIN ptab_petition_refs pr ON pr.trial_number = pp.trial_number
      WHERE coalesce(pp.trial_number,'') <> '' AND (pr.trial_number IS NULL OR coalesce(pr.grounds_v,0) < ${GROUNDS_V})
    ) q`;
  return rows[0] ? rows[0].n : 0;
}
...
export async function listPtabFwdBrief() {
  ...
  const { rows } = await sql`SELECT trial_number, trial_type, patent_number, petition_date,
    institution_date, fwd_date, outcome, dd_decision FROM ptab_fwd
