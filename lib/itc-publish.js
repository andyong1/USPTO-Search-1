...
import { documentsForDetail, getParties, getOutcomeBrief } from './itc-db.js';
...
export async function publishInvestigationDocs(number, meta) {
  ...
  const p = await getParties(number);
  const o = await getOutcomeBrief(number);
  const payload = {
    investigationNumber: number,
    publicNumber: meta ? meta.publicNumber : null,
    title: meta ? meta.title : null,
    phases: meta ? meta.phases : [],
    holdingSummary: o && o.ai_summary ? o.ai_summary : null,
    parties: p ? {
      complainants: p.complainants || [], respondents: p.respondents || [],
      assertedPatents: p.asserted_patents || [], accusedProducts: p.accused_products || null,
      requestedRemedies: p.requested_remedies || [], confidence: p.parties_confidence || null,
    } : null,
