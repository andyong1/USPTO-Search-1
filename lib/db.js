...
export async function ensureSchema() {
  ...
  // Whether the petition decision PDF includes a 35 U.S.C. 325(d) analysis.
  await sql`ALTER TABLE reexam_post_petitions ADD COLUMN IF NOT EXISTS decision_325d boolean`;
  await sql`ALTER TABLE reexam_post_petitions ADD COLUMN IF NOT EXISTS decision_parsed boolean NOT NULL DEFAULT false`;
...
export async function recordPostPetition(appNum, p) {
  ...
  await sql`
    ...
    ON CONFLICT (application_number) DO UPDATE SET
      ...
      -- When the decision document changes (or first appears), re-parse it for 325(d).
      decision_parsed = CASE WHEN reexam_post_petitions.decision_doc_id IS DISTINCT FROM EXCLUDED.decision_doc_id THEN false ELSE reexam_post_petitions.decision_parsed END,
...
// Petition decisions whose PDF hasn't been parsed for 325(d) yet.
export async function getPetitionDecisionsToParse(limit) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT application_number, decision_doc_id
    FROM reexam_post_petitions
    WHERE decision_doc_id IS NOT NULL AND COALESCE(decision_parsed, false) = false
    ORDER BY found_at ASC
    LIMIT ${limit}`;
  return rows;
}

export async function setPetitionDecision325d(appNum, is325d) {
  await ensureSchema();
  await sql`
    UPDATE reexam_post_petitions SET decision_325d = ${is325d}, decision_parsed = true
    WHERE application_number = ${appNum}`;
}
...
export async function listPostOrderPetitions() {
  ...
  const { rows } = await sql`
    SELECT p.application_number, p.order_date,
           ...
           p.decision_doc_id, p.decision_date, p.decision_outcome, p.decision_325d,
