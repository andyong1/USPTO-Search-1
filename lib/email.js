...
export async function sendDigest(newDocs) {
  const apiKey = process.env.RESEND_API_KEY;
  const from = process.env.DIGEST_FROM;
  const to = process.env.DIGEST_TO;
  ...
  if (!newDocs.length) return { skipped: true, reason: 'Nothing new to report.' };
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from,
      to: to.split(',').map((s) => s.trim()).filter(Boolean),
      subject: `USPTO: ${newDocs.length} new filing${newDocs.length !== 1 ? 's' : ''} in your tracked proceedings`,
      html: buildHtml(newDocs),
    }),
  });
