...
export async function ensureSchema() {
  ...
  // several queries) await one migration run instead of each re-running the full
  // DDL. On failure the promise is cleared so the next call retries.
  if (!schemaPromise) schemaPromise = runMigrations().then(() => { schemaReady = true; }).finally(() => { schemaPromise = null; });
...
async function runMigrations() {
  ...
  // Classification is done from the FWD PDF in a separate pass; classified_v lets
  // us reprocess everything when the classifier changes.
...
export async function upsertPtabFwdMeta(r) {
  ...
  await sql`
    ...
    ON CONFLICT (trial_number) DO UPDATE SET
      ...
      institution_date = EXCLUDED.institution_date, fwd_date = EXCLUDED.fwd_date,
      fwd_doc_id = EXCLUDED.fwd_doc_id, fwd_pdf_url = EXCLUDED.fwd_pdf_url`;
}

// FWDs still needing classification at the current classifier version.
export async function getPtabFwdToClassify(limit, currentV) {
  await ensureSchema();
  const { rows } = await sql`
    SELECT trial_number, fwd_pdf_url FROM ptab_fwd
    WHERE (classified_v IS NULL OR classified_v < ${currentV}) AND coalesce(fwd_pdf_url, '') <> ''
...
export async function setPtabFwdOutcome(trial, outcome, detail, source, v) {
  await sql`UPDATE ptab_fwd SET outcome = ${outcome}, outcome_detail = ${detail || ''}, text_source = ${source || ''}, classified_v = ${v}
            WHERE trial_number = ${trial}`;
}
// Promote rows that were classified from the standardized caption to version v
// without re-fetching their PDFs: the caption logic is unchanged across classifier
// versions, so those outcomes are stable. Only fallback/other/error rows re-run.
export async function promoteCaptionClassified(v) {
  await ensureSchema();
  const { rowCount } = await sql`UPDATE ptab_fwd SET classified_v = ${v}
    WHERE (classified_v IS NULL OR classified_v < ${v}) AND outcome_detail LIKE 'caption:%'`;
  return rowCount || 0;
}
...
export async function listRecentDeterminations(limit) {
  ...
  const { rows } = await sql`
    SELECT d.application_number, d.document_identifier, d.determination_type, d.official_date,
           d.group_art_unit, d.examiner_name, d.requester_type, d.found_at, w.filing_date,
           c.cert_doc_id, c.cert_date, c.nirc_doc_id, c.nirc_date, c.outcome_summary,
           c.claims_confirmed, c.claims_cancelled, c.claims_amended, c.claims_new,
           tc.tech_center
    FROM reexam_determinations d
    LEFT JOIN reexam_watch w ON w.application_number = d.application_number
    LEFT JOIN reexam_conclusions c ON c.application_number = d.application_number
